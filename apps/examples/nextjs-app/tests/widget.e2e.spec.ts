...
// End-to-end in a real browser: the example boots `next dev`, instrumentation.ts boots the mandarax
...
// proves the whole chain — withMandarax → register → client widget → engine — works in Next.js.
test('mandarax widget mounts in the Next.js app and connects to the engine', async ({page}) => {
  await page.goto('/')
