...
function App() {
  return (
    <main className="page-wrap px-4 pb-10 pt-10 sm:pt-14">
      <section className="island-shell island-raised rise-in relative overflow-hidden rounded-[2rem] px-6 py-10 sm:px-10 sm:py-14">
        <div className="pointer-events-none absolute -left-24 -top-28 h-72 w-72 rounded-full bg-[radial-gradient(circle,var(--accent-soft),transparent_66%)]" />
        <div className="pointer-events-none absolute -bottom-28 -right-24 h-72 w-72 rounded-full bg-[radial-gradient(circle,var(--hero-b),transparent_66%)]" />
        <div className="relative grid gap-10 lg:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)] lg:items-center">
          <div>
            <span className="eyebrow mb-5">
              <span className="eyebrow-dot" />
              TanStack Start base template
            </span>
            <h1 className="display-title mb-5 max-w-3xl text-4xl font-bold text-[var(--sea-ink)] sm:text-6xl">
            ...
            <div className="flex flex-wrap gap-2.5">
              <a href="/about" className="btn btn-primary btn-round no-underline">
                About This Starter
              </a>
              <a
                ...
                className="btn btn-ghost btn-round no-underline"
              >
                Router Guide
              </a>
            </div>
          ...
          <aside className="island-shell rounded-[1.25rem] p-5 sm:p-6">
            <p className="island-kicker mb-3">Take the tour</p>
      ...
      <section className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {FEATURES.map(([title, desc], index) => (
          <article key={title} className="feature-card rise-in" style={{animationDelay: `${index * 70 + 60}ms`}}>
            <span className="feature-mark mb-3 mono">{index + 1}</span>
      ...
      <section className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
        <div className="island-shell rounded-[1.5rem] p-6 sm:p-7">
          <div className="section-head mb-4">
            <h2 className="section-title">Quick start</h2>
            <p className="section-hint">Three files to make it yours</p>
          </div>
          <ul className="m-0 flex list-none flex-col gap-3 p-0">
            {QUICK_START.map(([file, desc]) => (
        ...
        <div className="island-shell flex flex-col justify-between gap-5 rounded-[1.5rem] p-6 sm:p-7">
          <div>
            <p className="island-kicker mb-2">Playground</p>
            <h2 className="display-title mb-2 text-2xl font-bold text-[var(--sea-ink)]">
              Poke at every control at once.
            </h2>
          ...
          <a href="/form" className="btn btn-primary btn-round self-start no-underline">
            Open the form demo
          </a>
        </div>
      </section>
