...
export function makeSessionClient(opts: {rpc: RpcClient; apiBase: string; sessionId: string}): SessionClient {
  const rpc = opts.rpc
  const initial = isSessionId(opts.sessionId) ? opts.sessionId : null
  const [sessionId, setSessionId] = createSignal<SessionId | null>(initial)
