...
in the file — it's used for "conciv is thinking…" / "conciv replied."). Reuse it to announce failure.
...
| ------------------------------------------------- | ------------------------------------------------------------ | ------------------- |
| Build widget bundle (tests load the built bundle) | `pnpm turbo run build --filter=@conciv/widget`               | exit 0              |
| Widget tests (real browser)                       | `pnpm --filter @conciv/widget exec vitest run`               | all pass            |
| Single test by name                               | `pnpm --filter @conciv/widget exec vitest run -t "Compress"` | matching tests pass |
| Typecheck                                         | `pnpm turbo run typecheck --filter=@conciv/widget`           | exit 0              |
| Lint                                              | `pnpm --filter @conciv/widget lint`                          | exit 0              |

IMPORTANT: the widget integration tests run a real browser against the **prebuilt** bundle
(`packages/widget/dist/conciv-widget.global.js`). You MUST rebuild the bundle (first command) after every
...
**Verify**: `pnpm turbo run typecheck --filter=@conciv/widget` → exit 0.
...
**Verify**: `pnpm turbo run typecheck --filter=@conciv/widget` → exit 0, and `pnpm --filter @conciv/widget lint` → exit 0.

### Step 3: Rebuild the bundle

**Verify**: `pnpm turbo run build --filter=@conciv/widget` → exit 0.
...
**Verify**: `pnpm --filter @conciv/widget exec vitest run -t "Compress"` → all Compress tests pass,
including the new 409 one. Then `pnpm --filter @conciv/widget exec vitest run` → full widget suite passes.
...
- Verification: `pnpm --filter @conciv/widget exec vitest run` → all pass (existing count + 1 new).
...
- [ ] `pnpm turbo run typecheck --filter=@conciv/widget` exits 0
- [ ] `pnpm turbo run build --filter=@conciv/widget` exits 0
- [ ] `pnpm --filter @conciv/widget exec vitest run` exits 0; the new 409 test exists and passes
- [ ] `pnpm --filter @conciv/widget lint` exits 0
