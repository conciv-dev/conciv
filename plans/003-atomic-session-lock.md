...
**Verify**: `pnpm turbo run typecheck --filter=@aidx/core` → exit 0.
...
**Verify**: `pnpm turbo run typecheck --filter=@aidx/core` → exit 0; `pnpm --filter @aidx/core lint` → exit 0.
...
**Verify**: `pnpm --filter @aidx/core exec vitest run lock` → all lock tests pass (existing + 2 new).

### Step 4: Full core verification

**Verify**: `pnpm turbo run test --filter=@aidx/core` → all pass.
...
- Verification: `pnpm turbo run test --filter=@aidx/core` → all pass.
...
- [ ] `pnpm turbo run typecheck --filter=@aidx/core` exits 0
- [ ] `pnpm turbo run test --filter=@aidx/core` exits 0; the 2 new lock tests pass
- [ ] `pnpm --filter @aidx/core lint` exits 0
