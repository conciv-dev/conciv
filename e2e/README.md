...
No app here may have a `prepare` script. `prepare` runs during `pnpm install`, before any
`@conciv/*` dist exists, and every app's vite config imports `@conciv/it`, so the config fails to
load and the install log fills with `ERR_MODULE_NOT_FOUND`. Anything a framework needs generated
(SvelteKit's `.svelte-kit`, for example) is already generated by its own `build`/`dev`, which run
after `turbo build`.
