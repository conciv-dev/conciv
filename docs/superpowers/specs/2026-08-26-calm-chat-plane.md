...
**When the landing runs.** A new thread (new viewport element, or neither edge key matching the last landing) always lands. The same thread re-lands on `totalSize()` changes only while the landing is still converging: the offset we last wrote is still in place, or the viewport is geometrically at the end (`instance.isAtEnd()`). One shot is not enough on its own, because the rows at the end mount after the first landing and measure taller than their estimates, so the end moves after we land — measured on a 60-turn mount, first landing at 4037 with the end at 4525 one frame later. Once the reader is away from the end the landing never runs again; the condition is geometry, not a flag. The send gesture and the "Latest" button call the same `landOnEnd`.
