...
- Motion: one `whileInView` fade-up (12px, 400ms, ease-out, `once`) owned by exactly: HeroCopy, PrinciplesStrip, CapabilitySection, HowItWorks, OpenSourceStrip. ProductFrame, nav, footer are static. Disabled under `prefers-reduced-motion`. Motion rule: no other entrance/ambient/scroll-driven animation anywhere on the page. Interaction feedback (hover color, focus ring, tab switch, copy state) is instant or ≤150ms opacity/color only — no layout/transform transitions, no animated tab pill, no shiki magic-move, no gradient edge fades. Only exception: the demo's own gsap ghost cursor inside ProductFrame (`gsap`/`@gsap/react` stay).
...
New: `install-command.tsx`, `product-frame.tsx`, `principles-strip.tsx`, `capability-section.tsx`, `open-source-strip.tsx`, `reveal.tsx`.
