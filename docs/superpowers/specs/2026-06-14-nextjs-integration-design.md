...
Let users run the conciv dev agent inside a Next.js app with the least friction Next physically
...
conciv integrates with Vite via a bundler hook (`transformIndexHtml`) because in Vite the
...
No proxy, no `conciv dev`, no React component, no `layout.tsx` edit. Integration is three
"instructions" touchpoints, all framework conventions:

```ts
// next.config.ts
import {withConciv} from '@conciv/plugin/nextjs'
export default withConciv({
  /* user config */
})
...
- `withConciv(nextConfig)` — synchronizes the port between server and client (see below) and is the
...
- `withConciv({ port? })` resolves a port (default e.g. `41700`, user-overridable), exposes it as
  `env.NEXT_PUBLIC_CONCIV_PORT` via Next's `env` config so it inlines into the client bundle, and as
  a non-public var for `register()` to read.
- `register()` boots the engine on that fixed port.
- the client entry reads `process.env.NEXT_PUBLIC_CONCIV_PORT` and points the widget at
  `http://127.0.0.1:<port>`.

If the port is taken, fail loudly with a clear message telling the user to set `withConciv({port})`.

## Components / changes

### New: `@conciv/plugin/nextjs` (and `/nextjs/widget` subpath)

- `withConciv(nextConfig)` — returns a Next config with `env.NEXT_PUBLIC_CONCIV_PORT` set; carries
  conciv options (harness, testRunner, enabled, port, stateRoot) for `register` to consume (via a
...
### Touch: `@conciv/core` engine — allow a fixed port
...
### Touch: `@conciv/widget` — apiBase without a meta tag

`mount.tsx` resolves apiBase from `<meta name="pw-api-base">`. Add a fallback: accept an explicit
apiBase (e.g. read `window.__CONCIV_API_BASE__` or a small `mount({apiBase})` export). The Next
...
- Pages Router.
- Proxy mode (`conciv dev`) — viable alternative for true zero-edit, deferred.
...
## Open questions
None blocking. Port default value and the exact mechanism for passing conciv options to `register`
