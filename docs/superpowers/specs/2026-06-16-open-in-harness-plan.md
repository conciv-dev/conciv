...
type HarnessLaunchContext = {
  ...
  mcpUrl: string | null         // the aidx MCP-over-HTTP endpoint (for tool parity)
...
## Carry-over (decided: model + aidx MCP tools)
...
same aidx page tools (`aidx_ui` / `aidx_page` / `aidx_test`) as the in-widget agent.
...
- **codex:** `codex resume <id>` + `-m <model>` confirmed. Codex has NO `--mcp-config` flag - MCP is
  ...
  `-m`, and MCP only via `-c 'mcp_servers.aidx…'` overrides (heavier) OR model-only for v1.

## Open verifications (do at build time, before relying on them)

- **MCP endpoint lifetime:** confirm the aidx MCP-over-HTTP server is up for the dev server's
...
- Manual: run the dev server, click the button, confirm the harness CLI opens in the terminal at the
  right cwd, resumes the session, on the selected model, with the aidx tools available; confirm the
