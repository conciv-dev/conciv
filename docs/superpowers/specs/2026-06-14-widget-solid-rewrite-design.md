...
Rewrite `@conciv/widget` from React to SolidJS, removing all React. Behavior stays 1:1 with the
...
- Keep: `@tanstack/ai-client` (framework-agnostic types), `@tanstack/ai`, `marked`, `shiki`, `@conciv/protocol`
...
| `mount.tsx`      | Solid `render(() => <ChatFeature/>, container)` into the shadow root (replaces `createRoot().render`). Preserve the `__CONCIV_RENDER_TEST_CARD__` test seam and the `probeChatAvailable` gate. Export `mountWidget` and auto-mount on load (unchanged contract). |
...
- `vite.config.ts`: `@vitejs/plugin-react` → `vite-plugin-solid`; drop the React-only
  `define: {'process.env.NODE_ENV': ...}`. Keep the dual ESM + IIFE lib build (`ConcivWidget`,
  `conciv-widget.global.js`), `cssCodeSplit: false`, `styles.css?inline`, sourcemaps.
