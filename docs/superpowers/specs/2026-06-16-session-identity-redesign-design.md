...
- **header id** — the `conciv-session-id` we send (`'default'` or a client `crypto.randomUUID()`). Stable from t=0.
...
They coincide only when you switch to an existing row; for any conciv-started session they diverge. A server-side map (`headerId → token`, `.conciv/chat-sessions.json`) bridges them. Symptoms of the split:
...
- Our id is `conciv_<uuid>`. The `conciv_` prefix makes "ours vs a raw harness id" decidable at a glance and in code.
...
```
SessionRecord {
  id: string                 // conciv_<uuid> — primary key
  ...
  origin: 'chat' | 'agent' | 'external'
                             // chat = born in widget; agent = handed off via CONCIV_SESSION_ID;
...
- **fs layout:** `.conciv/sessions/<previewId>/conciv_<uuid>.json`, key = our id, value = `SessionRecord`.
...
- **One resolve seam.** `POST /api/chat/session/resolve { id? }` always returns `{ sessionId: conciv_<uuid> }`:
  - **no id** → mint a new record `{ id: conciv_<uuid>, harnessSessionId: null, origin: 'chat' }` (new session).
  ...
  - **our `conciv_` id** → return it, mark last-active.
...
- **Agent hand-off:** launched with `CONCIV_SESSION_ID = <harness id>` → at boot, ensure a record exists wrapping it (`origin: 'agent'`, `harnessSessionId` pre-filled).
- **External discovery:** `GET /sessions` is read-only — it returns our records (id = `conciv_…`) plus harness transcripts with no record yet (row id = the raw harness id, the only id they have). The client hands whichever row id to `resolve`, which normalizes it to an `conciv_` id (adopting the external transcript on first open). No write-on-list.

## Client identity invariant (hard rule)

**The widget only ever knows, stores, and transmits our `conciv_` id. The harness id is never used for any backend communication, routing, selection, or persistence — it may appear only as read-only display ("extra info").**

- **Comms chokepoint:** every request goes through the `defineClient` instance (`session-client.ts`); the `conciv-session-id` header and all request bodies carry our id, only. This is the single seam to guard.
- **Selection/state keyed by our id:** session list rows, the active-row match, surface rows, quick-terminal panes, and the persisted active id are all our `conciv_` id. The `activeToken` debugging hack is deleted — there is no token in client routing.
...
- **One sanctioned exception — the `resolve` endpoint.** External/discovered rows may carry a raw harness id. The client never _routes_ by it; it passes the row id once to `POST /api/chat/session/resolve`, which returns the canonical `conciv_` id. From that point every request uses the `conciv_` id. `resolve` is the only endpoint that accepts a non-`conciv_` id, and its sole job is to normalize any id to ours.

## API surface (reshaped)

All routes key by our id (the `conciv-session-id` header), never the token.

- `POST /api/chat/session/resolve { id? }` → returns `{ sessionId: conciv_<uuid> }`. The **only** id-normalization seam: no id → mint new; harness id → find-or-create wrapper; our id → return + mark last-active. (Replaces the old `/session/new`, mint, and adopt.)
...
```ts
// @conciv/widget/session-client.ts
...
const {sessionId} = await client.resolve() // new session → conciv_ id (no args — body is all-optional)
...
- `engine.ts`: revisit `CONCIV_SESSION_ID` child env (now our id).
...
- `session-selector.tsx`: `activeId` = our id, matches our rows directly; selecting any row → `resolve(rowId)` → set the returned `conciv_` id (this is where an external row gets adopted).
...
- **Widget (real browser, Playwright IT):** new session → chat → **reload restores it**; switch → reload restores; adopt an external row → it gets an `conciv_` id and persists; rename persists; running indicator reflects locks. (Keep the reload regression test already drafted, re-pointed at the new model.)
...
- **List carries two id formats** transiently (our `conciv_` ids vs raw harness ids for never-opened external rows). The `conciv_` prefix keeps it unambiguous, and `resolve` collapses it to our id on first open.
