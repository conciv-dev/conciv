...
- **harness token** — the harness's own session id (`harnessId`). `null` until the first turn mints it; the session *list* is keyed by it.
...
- **Sessions don't persist / "+New" vanishes on refresh** — the active id was sometimes compared against a list keyed by the *other* id space.
...
> **Our id is the primary key for everything. The harness's id is an attribute of a session (`harnessSessionId`), passed to the harness *only* for `--resume`. Nothing else ever keys off it.**
...
- The harness still owns *its* identity; we never force it (no `--session-id`). We adopt the id it mints.
...
- **Locks stay separate:** `agent.<our-id>.lock` (pid + role), scanned by readdir for the running indicator; a dead pid = auto-free crash recovery. Transient lock state is deliberately *not* in the durable record.
...
- **One sanctioned exception — the `resolve` endpoint.** External/discovered rows may carry a raw harness id. The client never *routes* by it; it passes the row id once to `POST /api/chat/session/resolve`, which returns the canonical `aidx_` id. From that point every request uses the `aidx_` id. `resolve` is the only endpoint that accepts a non-`aidx_` id, and its sole job is to normalize any id to ours.
...
**3. The client — `defineClient`** — a per-instance closure that *owns* its active session id. Each pane (and the modal) holds its own. Routes are declared **inline** via a generic `route(spec)` — no registry object, no `Object.fromEntries`, no cast. Method types are *inferred* from the schemas, so nothing is typed twice:
```ts
...
export function defineClient(deps: {apiBase: string}) {
  ...
  function route<Res extends z.ZodTypeAny>(
    spec: {method: 'GET' | 'DELETE'; path: string; response: Res; sendsSessionId?: boolean},
  ): () => Promise<z.infer<Res>>
  function route<Req extends z.ZodTypeAny, Res extends z.ZodTypeAny>(
    spec: {method: 'POST'; path: string; request: Req; response: Res; sendsSessionId?: boolean},
  ): (...args: Args<z.infer<Req>>) => Promise<z.infer<Res>>
  function route(spec: {method: string; path: string; request?: z.ZodTypeAny; response: z.ZodTypeAny; sendsSessionId?: boolean}) {
    return (body?: unknown) => {
  ...
  return {
    ...
    resolve:  route({method: 'POST', path: '/api/chat/session/resolve', request: ResolveRequestSchema, response: ResolveResponseSchema}),
    ...
    rename:   route({method: 'POST', path: '/api/chat/sessions/title', request: RenameSessionSchema, response: RenameResponseSchema}),
    launch:   route({method: 'POST', path: '/api/chat/launch', request: ChatLaunchRequestSchema, response: ChatLaunchSchema, sendsSessionId: true}),
    remove:   route({method: 'DELETE', path: '/api/chat/session', response: OkSchema, sendsSessionId: true}),
  }
}
```
`defineClient`'s return type is *inferred* from the `route(...)` calls — no annotation, no `SessionClient` type to maintain, and `client.nope` is a compile error because it isn't a property. (`route`'s implementation signature is the conventional broad overload impl; the two public overloads above it are exact.)
...
const onSwitch = async (rowId: string) => {          // switch / open external row
...
4. Widget: `defineClient` (`session-client.ts`) — per-instance closure owning its session id, routes declared inline from the shared schemas; replaces `chat-api.ts`; branded id enforces the our-id-only invariant at compile time.
- **Verify:** typecheck — a raw harness `string` is rejected everywhere except `resolve`.

**Phase 3 — Widget rewiring.**
5. `widget-shell` (persist our id, delete `activeToken`), `session-selector` (key by our id, `resolve` on switch/open-external), `quick-terminal`, `chat-panel`, `session-store-client`, `session-info` (harness id display-only).
- **Verify:** widget browser IT (below).

**Phase 4 — Harness enrichment + cleanup.**
6. Extend claude `listSessions`/transcript parse for the enrichment fields; final dead-code sweep; lint + build.
