...
(`aguiCustomFor` → `AIDX_UI_EVENT` → `useChat({onCustomEvent})`) used by generative UI; the
...
claude/codex CLI stdout (NDJSON)
  ...
  runAgui spine  ──step(event)──▶  content StreamChunks (text/tool/…)   [unchanged]
        ...
        └──extractUsage(event)──▶ Partial<UsageSnapshot>
                  ...
                  │ emit on change
                  ▼
        aguiUsageFor(snapshot)  ─▶  CUSTOM StreamChunk {name: "aidx-usage"}
                  │ (SSE transport, same as aidx-ui)
                  ▼
...
The wire contract. Mirrors `ui-types.ts` (`AIDX_UI_EVENT` / `aguiCustomFor`).
...
// `aidx-usage` CUSTOM chunk whenever the merged snapshot changes. A harness that omits
...
const onCustom = (eventType: string, data: unknown) => {
  if (eventType === AIDX_UI_EVENT) return onAidxUi(data)
...
- **Node unit (`packages/harness/test`)**: extend `claude-decode` / `codex-decode` tests to
  ...
  assert the emitted `aidx-usage` CUSTOM chunk(s) carry the normalized fields, including
