...
Expand the conciv widget with a richer layout system, drawing the floating-button
...
- Picture-in-Picture: pop either layout into a separate OS window (ported from
  Devtools).
- All config namespaced under `ConcivConfig.widget`.
...
| Question              | Resolution                                                                                                     |
| --------------------- | -------------------------------------------------------------------------------------------------------------- |
| Config shape          | Namespaced under `ConcivConfig.widget`; `modal` and `quickTerminal` are independent toggles (both can be on)   |
...
─────────────────────────         ──────────────         ────────────────
ConcivConfig {                       htmlTags() adds        mount.tsx reads <meta pw-widget>:
...
```ts
conciv() // both on (defaults)
conciv({widget: {modal: {position: 'top-left'}}}) // both on, move the button
conciv({widget: {quickTerminal: false}}) // modal only
conciv({widget: {modal: false, quickTerminal: {hotkey: ['Mod+`', 'Control+k']}}}) // qt only, custom keys
```
...
- Drag (our addition on top of the enum): pointerdown on the FAB tracks the
  ...
  (`conciv-fab-position`). This is the superset the user asked for. Devtools itself
  only sets position by enum, so the snap-on-drop is conciv-specific but built on
  their preset model.
...
Persist the panel height to localStorage (`conciv-modal-height`).

### Picture-in-Picture (ported from TanStack Devtools, MIT)

Source: `context/pip-context.tsx`. Port the provider near-verbatim:

- `requestPipWindow(settings)`: `window.open('', 'conciv-widget', '${settings},popup')`;
...
- Persisted to localStorage: quick-terminal height (`conciv-qt-height`), modal
  panel height (`conciv-modal-height`), FAB position (`conciv-fab-position`).
...
1. `conciv()` (no widget config) enables BOTH layouts: modal FAB bottom-right and
   the quick terminal on `Mod+\``.
2. `conciv({ widget: { quickTerminal: { hotkey: ['Mod+`', 'Control+k'] } } })`toggles the sheet on either binding;`quickTerminal: false` disables it.
3. `conciv({ widget: { modal: { position: 'top-left' } } })` starts the FAB top-left;
