...
**Tech Stack:** `@conciv/protocol` (capability + config + schema), `@conciv/harness` (args + decode), `@conciv/core` (config resolution), `@conciv/tool-ui` (`DoneCard` from Plan B), `@conciv/widget`. Both CLI behaviors were verified live (see the spec); re-verify in the conciv pipeline (Task 6).
...
- `packages/protocol/src/config-types.ts` (modify) — `doneCard?: boolean` on `ConcivConfig`.
...
In `packages/protocol/src/config-types.ts`, add to `ConcivConfig`:
...
Run: `pnpm --filter @conciv/protocol exec vitest run test/done-card.test.ts && pnpm --filter @conciv/harness typecheck`
...
Run: `grep -n "systemPrompt\|enabled\|resolveConfig\|ConcivConfig" packages/core/src/config.ts | head`
...
```ts
import {DONE_CARD_JSON_SCHEMA} from '@conciv/protocol/tool-types'
...
Run: `pnpm --filter @conciv/harness exec vitest run test/structured-output.test.ts`
...
Import `DoneCard` from `@conciv/tool-ui` and `DoneCardSchema` from
`@conciv/protocol/tool-types`. Add a `rawText(part)` helper returning `part.raw ?? ''`.
...
Set `doneCard: false` in the conciv config; confirm the CLI args no longer include
...
Run: `pnpm turbo run typecheck build test --filter=@conciv/protocol --filter=@conciv/harness --filter=@conciv/core --filter=@conciv/widget --filter=@conciv/tool-ui`
Expected: green.
