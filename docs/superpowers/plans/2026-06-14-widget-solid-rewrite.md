...
**Goal:** Rewrite `@conciv/widget` from React to SolidJS, removing all React, keeping behavior 1:1 (Solid-idiomatic cleanup allowed), with the existing Chromium playwright IT as the parity gate.
...
// One entry (mount.tsx) ships two ways: an ESM module (@conciv/widget) and a self-contained IIFE
...
export default defineConfig({
  ...
  build: {
    lib: {
      ...
      formats: ['es', 'iife'],
      name: 'ConcivWidget',
      fileName: (format) => (format === 'iife' ? 'conciv-widget.global.js' : 'mount.js'),
    },
    cssCodeSplit: false,
    emptyOutDir: true,
    sourcemap: true,
...
export const Markdown: Component<Props> = (props) => {
  ...
  return <div class="conciv-md" innerHTML={html() ?? marked.parse(props.text, {async: false})} />
}
...
Run: `pnpm --filter @conciv/widget exec tsc --noEmit -p tsconfig.json 2>&1 | grep markdown`
...
// import the spec types from @conciv/protocol/ui-types as the React version did
...
Run: `pnpm --filter @conciv/widget exec tsc --noEmit -p tsconfig.json 2>&1 | grep gen-ui`
...
export const TestCard: Component<{apiBase: string; onFix: (...a: never[]) => void; result: TestRunResult | null}> = (
  ...
  return <div class="conciv-test-card">{/* ... */}</div>
}
```
...
Run: `pnpm --filter @conciv/widget exec tsc --noEmit -p tsconfig.json 2>&1 | grep test-card`
...
Run: `pnpm --filter @conciv/widget exec tsc --noEmit -p tsconfig.json 2>&1 | grep chat-shell`
...
Preserve the public contract: exports `mountWidget`, auto-mounts on load, guards against double-mount (`[data-conciv-root]`), probes via `probeChatAvailable(apiBase)` before mounting chat, and exposes the `__CONCIV_RENDER_TEST_CARD__` test seam (the IT calls it).
...
type TestSeam = {__CONCIV_RENDER_TEST_CARD__?: () => void}
...
export function mountWidget(): void {
  ...
  w.__CONCIV_RENDER_TEST_CARD__ = () => mountTestCardForTest(root, apiBase)
...
Run: `pnpm --filter @conciv/widget build`
Expected: emits `dist/conciv-widget.global.js` and `dist/mount.js`; no react in the bundle —
verify: `grep -c "react-dom" packages/widget/dist/conciv-widget.global.js` → `0`.

- [ ] **Step 3: Run the browser IT (parity gate)**

Run: `pnpm --filter @conciv/widget test`
...
Run: `pnpm --filter @conciv/widget lint`
Expected: PASS.

- [ ] **Step 5: Full workspace check (nothing downstream broke)**

Run: `pnpm build && pnpm test && pnpm typecheck`
Expected: all tasks succeed (the plugin serves the rebuilt `conciv-widget.global.js`).
...
- The `__CONCIV_RENDER_TEST_CARD__` seam and `pw-api-base` meta read are preserved (T6) so the IT keeps working.
