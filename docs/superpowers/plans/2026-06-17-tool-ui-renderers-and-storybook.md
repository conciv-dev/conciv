# Tool UI — Plan B: `@opendui/aidx-tool-ui` package + renderers + Storybook
...
Run: `pnpm install`
Expected: `@opendui/aidx-tool-ui` is linked; no peer errors.
...
git commit -m "chore(tool-ui): scaffold @opendui/aidx-tool-ui package"
```
...
```css
/* tokens.css — shared aidx design tokens; resolves in the widget shadow root and on :root */
:host,
:root {
...
```css
@import '@opendui/aidx-tool-ui/tokens.css';
```

Add `"@opendui/aidx-tool-ui": "workspace:*"` to `packages/widget/package.json` dependencies.
...
git commit -m "refactor(tokens): move --pw-* tokens into @opendui/aidx-tool-ui"
```
...
import type {ToolKind} from '@opendui/aidx-protocol/tool-types'
...
Run: `pnpm --filter @opendui/aidx-tool-ui storybook`
...
Run: `pnpm --filter @opendui/aidx-tool-ui storybook` → verify; then
...
- Spec coverage: creates `@opendui/aidx-tool-ui` with the registry keyed on `ToolKind`, all nine
