...
**Tech Stack:** SolidJS, zod v4, h3 (server), TanStack AI tool defs, jiti (server-half loading), tsdown (package build), babel strip transform (`@mandarax/extension-build` — extracted from the dev-only plugin so shipped packages never depend on it), Playwright (browser ITs), vitest (unit/node ITs), turborepo, oxlint/oxfmt.
...
> **Decided architecture — the dev-only plugin is NEVER a dependency of a shipped package.** `stripServerHalf` (`plugin/src/core/strip-server.ts:62`) is a pure babel transform with no vite/`serve` coupling, but it currently lives inside `@mandarax/plugin`, whose vite hook is `apply: 'serve'` (dev-only). The plugin must never be referenced by any shipped package or reach a production bundle. So **extract `stripServerHalf` into a standalone build-util package** that both consumers depend on — neither shipped package touches the plugin. `@mandarax/test-runner` gets NO `browser` condition; its node-free `/events` subpath is the ONLY client-safe entry, and the Build IT (not the strip) is the authoritative gate against node code reaching the browser.

**Prerequisite Task 13b — extract the strip util:**

- Create: `packages/extension-build/` — a real package (`src/strip-server.ts` moved verbatim from `plugin/src/core/strip-server.ts`, `tsconfig`, `vitest`) exporting `stripServerHalf(code, filename)`. Pure babel; no vite, no `apply:'serve'`, no app-runtime code.
- Modify: `packages/plugin/src/core/` — import `stripServerHalf` from `@mandarax/extension-build` instead of the local file; add the dep; delete the local copy. The plugin's existing strip ITs (`plugin/test/strip-server.it.test.ts`) still pass.
- [ ] Move + re-point + green the plugin's strip ITs; commit `"refactor: extract stripServerHalf to @mandarax/extension-build (plugin stays dev-only)"`.

**Files:**

- Create: `packages/extension-test-runner/package.json` (deps: `@mandarax/extension`, `@mandarax/test-runner`, `@mandarax/ui-kit-system`, `@mandarax/api-client`, `zod`, `solid-js`, `lucide-solid`; devDeps: `@mandarax/extension-build`, `tsdown` — NOT `@mandarax/plugin`; `exports`: `.` → `node`/`default` `dist/extension.js`; `./client` → `dist/extension.browser.js` for the plugin to import in `extensionsModuleSource`), `tsconfig.json`, `tsdown.config.ts` (two entries; the browser entry runs `@mandarax/extension-build`'s `stripServerHalf` on `extension.ts`).
...
- [ ] **Steps 1–5:** scaffold; wire the dual-view tsdown build using `@mandarax/extension-build`; make the build IT pass; commit `"feat(extension-test-runner): package scaffold + browser/node split build IT"`.
