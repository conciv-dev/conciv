...
**Architecture:** A prebuilt Swift + ScreenCaptureKit binary is spawned by `@conciv/core` as a child of the dev server; it rides the dev-host (terminal/IDE) Screen Recording grant and writes a PNG. A new `POST /api/page/capture` core route exposes it. The widget calls it during a grab and attaches the PNG as an inline image content part on the next turn, reusing the existing image-delivery path. Every layer treats capture as optional: missing binary, non-macOS host, denied permission, or any spawn error falls back silently to the existing DOM-clone-only grab.
...
- `packages/capture-macos/build.sh` — builds the binary into `bin/<arch>/conciv-capture`.
- `packages/capture-macos/bin/arm64/conciv-capture` — prebuilt binary (committed; mac-only MVP).
- `packages/capture-macos/src/index.ts` — `resolveCaptureBinary()`: returns the host-arch binary path or null.
- `packages/capture-macos/package.json` — `@conciv/capture-macos`, `os: ["darwin"]`.
...
Run: `pnpm --filter @conciv/protocol test capture-types`
...
Run: `pnpm --filter @conciv/core test host-app`
...
OUT="$DIR/bin/$ARCH"
mkdir -p "$OUT"
swiftc "$DIR/src/main.swift" -o "$OUT/conciv-capture" \
  -target "$ARCH-apple-macos14.0" -parse-as-library \
  -framework AppKit -framework ScreenCaptureKit -framework CoreGraphics
codesign --force --sign - "$OUT/conciv-capture" # ad-hoc; rides the dev-host grant, no own identity
echo "built $OUT/conciv-capture"
```
...
./packages/capture-macos/bin/$(uname -m)/conciv-capture --out /tmp/conciv-smoke.png; echo "exit=$?"
```

Expected: a single JSON line. With Screen Recording granted to the host terminal and a browser window open: `{"ok":true,...}`, exit 0, `/tmp/conciv-smoke.png` exists. Without the grant: `{"ok":false,"reason":"permission"}`, exit 1.

- [ ] **Step 5: Verify the PNG visually (manual, one-time)**

Open `/tmp/conciv-smoke.png` and confirm it is a real browser window, not wallpaper. (If it is wallpaper, the host terminal lacks the grant — that is the `permission` path, expected.)
...
- Consumes: the built binary at `bin/<arch>/conciv-capture` from Task 3.
...
Run: `pnpm --filter @conciv/capture-macos test`
...
export function resolveCaptureBinary(): string | null {
  ...
  // dist/index.js -> ../bin/<arch>/conciv-capture
  const path = join(here, '..', 'bin', arch, 'conciv-capture')
...
```bash
#!/bin/bash
# Mimics conciv-capture: --out <path> [--mode ok|permission]. Writes a 1x1 PNG and the JSON line.
OUT=""; MODE="ok"
...
describe('captureWindow', () => {
  ...
  it('returns binary-missing when the path does not exist', async () => {
    const r = await captureWindow({}, {binaryPath: '/no/such/conciv-capture'})
...
import {
  ...
  type CaptureRequest,
  type CaptureResult,
} from '@conciv/protocol/capture-types'
import {resolveCaptureBinary} from '@conciv/capture-macos'
...
export async function captureWindow(req: CaptureRequest, opts: CaptureOpts = {}): Promise<CaptureResult> {
  ...
  const out = join(tmpdir(), `conciv-capture-${randomUUID()}.png`)
...
Note: add `@conciv/capture-macos` to `packages/core/package.json` `dependencies` (workspace) and `optionalDependencies` semantics are unnecessary because `resolveCaptureBinary` already null-guards; the import must not throw on non-darwin, so `index.ts` must have no top-level darwin-only side effects (it does not).

- [ ] **Step 4: Run tests to verify they pass**

Run: `pnpm --filter @conciv/core test page/capture`
Expected: PASS (4 tests).
...
Run: `pnpm --filter @conciv/core test api/page/capture`
...
```ts
import {CaptureRequestSchema} from '@conciv/protocol/capture-types'
...
Run: `pnpm --filter @conciv/widget test capture-screenshot`
Expected: FAIL — cannot resolve module.

- [ ] **Step 3: Write minimal implementation**

```ts
import type {CaptureResult} from '@conciv/protocol/capture-types'
...
Run: `pnpm --filter @conciv/widget test capture-e2e`
