...
**Goal:** A consumer drops a file in `mandarax/extensions/` and the widget picks it up live — no manual `window.__MANDARAX__` wiring. The unplugin discovers extension files, registers their client halves into the running widget, and hot-reloads on edit.

**Architecture:** Discovery + injection happen in the existing `@mandarax/plugin` vite hook (`makeViteHook`). A vite **virtual module** (`virtual:mandarax-extensions`) uses `import.meta.glob` to import every `mandarax/extensions/*` module and feed each default export to `window.__MANDARAX__.use(...)`. The widget's existing `installExtensionGlobal` (Slice 1) applies them live. The hard part — verified against the running example app, not assumed — is **injecting the virtual module as a real vite-graph module so HMR works on both the static (`transformIndexHtml`) and SSR (`makeWidgetInject`) paths**.
...
- **Static path:** core's `htmlTags()` (`packages/core/src/widget-tags.ts`) returns tags consumed by vite's `transformIndexHtml`. Vite _will_ transform an inline `<script type="module">` here, so `import 'virtual:mandarax-extensions'` resolves and gets HMR.
...
**Resolution to verify:** serve the extensions entry as a real URL from a vite middleware (same dev origin as the widget script) by delegating to `server.transformRequest('virtual:mandarax-extensions')`, and reference it with `<script type="module" src="/@mandarax/extensions.js">`. Confirm whether HMR fires through this path; if not, fall back to injecting via vite's module graph (e.g. add the entry to `server.moduleGraph` / use `/@id/` URL) so the HMR client registers. **Spike this against the running example before finalizing Task 2.**
...
- `apps/examples/tanstack-start/mandarax/extensions/blue.ts` **(create)** — a sample extension (plain `{id, clientFn}` shape; no runtime import needed yet) used by the e2e.
...
- Produces: `EXTENSIONS_VIRTUAL_ID = 'virtual:mandarax-extensions'`, `EXTENSIONS_RESOLVED_ID = '\0virtual:mandarax-extensions'`, `extensionsModuleSource(): string`.
- The source uses `import.meta.glob('/mandarax/extensions/*.{ts,tsx,js,jsx}', {eager: true})` and for each module's `default` calls a register helper that (a) pushes to `window.__MANDARAX__.queue` if `use` isn't ready, (b) calls `window.__MANDARAX__.use(ext)` if it is.
...
- [ ] **Step 3:** Typecheck the plugin: `pnpm turbo typecheck --filter=@mandarax/plugin`.
- [ ] **Step 4:** Commit.
...
**Files:** Create `apps/examples/tanstack-start/mandarax/extensions/blue.ts` + `apps/examples/tanstack-start/e2e/extensions.spec.ts`.

- [ ] **Step 1:** Sample extension (plain object shape, no runtime import):

```ts
// apps/examples/tanstack-start/mandarax/extensions/blue.ts
export default {
  id: 'blue',
...
- **Slice 3 — Server half + tools:** add `jiti` (ASK FIRST) to jiti-load `.server` halves in core boot; wire `toolDefinition().server()` + `registerTool` + `systemPrompt.append` into `@mandarax/core start`; ship a runtime `@mandarax/widget/extension` export (build change) so files can `import { defineExtension, toolDefinition }`.
- **Slice 4 — Catalog + legibility:** live `mandarax_ui catalog/scaffold/validate` from `TOKENS` + registries; the `mandarax-extensions` skill; `examples/extensions/`.
