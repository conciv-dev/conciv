...
> `@mandarax/core/src/api/<domain>/` with paths `/api/<domain>/…` and the entry file named after its
...
**Goal:** Extract the inline claude harness (currently wired inside `@mandarax/core` per Plan 1) into a dedicated `@mandarax/harness` package behind the capability-declaring `HarnessAdapter` interface, add a real **codex** proof adapter, ship **gemini-cli / opencode / pi** as capability-only stubs, and make `@mandarax/core` feature-detect by capability (degrade gracefully when `permissionGate`, `transcriptHistory`, `resume`, or file `systemPrompt` are absent).

**Architecture:** Adapter registry (`registerHarness` / `getHarness` / `listHarnesses`) over the `HarnessAdapter` contract from `@mandarax/protocol/harness-types`. Each adapter is a subfolder of named files (`<id>.ts` declares the adapter + capabilities; `args.ts`/`decode.ts`/`history.ts`/`system-prompt.ts` hold the ported behaviour). Adapters depend only on `@mandarax/protocol` and `@tanstack/ai` — never on the engine. `@mandarax/core` resolves `getHarness(config.harness ?? 'claude')` and wires routes conditionally on the adapter's `capabilities`.
...
- `@mandarax/protocol/harness-types` **exists** and exports `HarnessCapabilities`, `HarnessTurn`, `HarnessChild`, `HarnessAdapter` exactly as specified in the design's "Seam 1":

  ```ts
  // @mandarax/protocol/harness-types (created by Plan 1)
...
packages/harness/
  ...
  tsdown.config.ts            per-module entries; @tanstack/ai + @mandarax/protocol external
...
packages/core/                (edited — feature-detection wiring)
  package.json                + dependency "@mandarax/harness": "workspace:*"
  src/
...
## Task 1 — Scaffold `@mandarax/harness` + register in the workspace
...
- [ ] Write the **failing test** `packages/harness/test/registry.test.ts`:
  ...
  import {defineHarness, type HarnessAdapter} from '@mandarax/protocol/harness-types'
...
- [ ] **Run** (expect FAIL — package + module do not exist yet):
  ...
  ```
  pnpm --filter @mandarax/harness test
  ```

  Expected: command errors with `No projects matched the filters "@mandarax/harness"` (package not yet in the workspace). That counts as RED for this task — the package must exist before the test can even run.

- [ ] **Minimal impl** — create `packages/harness/package.json`:

  ```json
  {
    "name": "@mandarax/harness",
    "version": "0.0.0",
    "description": "Harness adapters (claude, codex, + stubs) behind the @mandarax/protocol HarnessAdapter contract.",
    "license": "MIT",
    "files": ["dist"],
    "type": "module",
    "exports": {
...
- [ ] Create `packages/harness/tsdown.config.ts` (each adapter is its own entry so the subpath exports resolve to named files; `@tanstack/ai` + `@mandarax/protocol` stay external):
...
- [ ] Create `packages/harness/src/registry.ts` (registry + self-registration of bundled adapters; uses a module-level `Map`, functions-not-classes, no IIFE). The adapter imports are added as each adapter is built — start with `claude` + `codex` (Tasks 2-3) and the three stubs (Task placeholder; created in Task 7-ish but referenced here from the start so `listHarnesses` includes them). For Task 1 RED→GREEN, create the registry referencing only the bundled adapters that will exist; if an adapter file is not yet created, comment its import + register line and uncomment it in the task that creates it.
  ...
  (substitute `codex`/`geminiCli`/`opencode`/`pi` + their ids accordingly). `defineHarness` (from Plan 1, `@mandarax/protocol/harness-types`) returns its argument typed as the exact literal `T extends HarnessAdapter`, so each export stays assignable to `HarnessAdapter` in the registry. The placeholder declares `transcriptHistory: false` with no `transcriptPath`/`parseHistory`, satisfying `defineHarness`'s dev-time invariant. This makes the registry compile and `listHarnesses()` return all five immediately; later tasks replace each placeholder body.
...
- [ ] **Run** the build + typecheck to confirm scaffolding is sound:
  ...
  ```
  pnpm --filter @mandarax/harness build && pnpm --filter @mandarax/harness typecheck
  ```
...
- [ ] **Commit**:
  ...
  git commit -m "feat(harness): scaffold @mandarax/harness package + registry
...
- [ ] Write the **failing test** `packages/harness/test/claude-args.test.ts`. Asserts the ported argv shape, that the PreToolUse hook `--settings` is added **only** when `permissionUrl` is present (the route supplies it iff `capabilities.permissionGate==='hook'`), and that `--append-system-prompt-file` / `--resume` are conditional:
  ...
  describe('claude buildArgs', () => {
    it('builds the headless stream-json argv with acceptEdits + allowed mandarax tools + add-dir', () => {
      ...
      expect(args).toContain('Bash(mandarax tools:*)')
      expect(args).toContain('Bash(mandarax ui:*)')
...
- [ ] **Run** (expect FAIL — `args.ts` exports nothing yet):
  ...
  ```
  pnpm --filter @mandarax/harness test claude-args
  ```
...
- [ ] **Minimal impl** — create `packages/harness/src/claude/args.ts` (ported verbatim from `claude-args.ts`, retyped onto `HarnessTurn`; `appendSystemPromptFile` → `systemPrompt`):
  ...
  // session when one is supplied. The agent's own `mandarax tools`/`mandarax ui` CLIs are
...
- [ ] **Run** (expect FAIL):
  ...
  ```
  pnpm --filter @mandarax/harness test claude-decode
  ```
...
- [ ] **Minimal impl** — create `packages/harness/src/claude/decode.ts`, ported verbatim from `claude-agui-stream.ts`, renaming the export `claudeToAguiEvents` → `decode` and the opts type to the protocol's `HarnessDecodeOpts` (`onSessionId` becomes required — core always supplies it):
  ...
  export async function* decode(lines: AsyncIterable<string>, opts: HarnessDecodeOpts): AsyncGenerator<StreamChunk> {
    const threadId = 'mandarax-chat'
    const runId = 'mandarax-run'
    const ids = {n: 0}
...
- [ ] **Run** (expect FAIL):
  ...
  ```
  pnpm --filter @mandarax/harness test claude-history
  ```
...
- [ ] **Run** the whole package suite + typecheck to confirm claude is fully ported:
  ...
  ```
  pnpm --filter @mandarax/harness test && pnpm --filter @mandarax/harness typecheck
  ```
...
- [ ] Write the **failing test** `packages/harness/test/codex-args.test.ts`:
  ...
  describe('codex buildArgs', () => {
    ...
    it('passes the system prompt via the instructions flag when provided (systemPrompt:flag)', () => {
      const args = buildArgs({...base, systemPrompt: 'you are mandarax'})
      const i = args.indexOf('--instructions') // VERIFY exact flag in Task 5
      expect(i).toBeGreaterThan(-1)
      expect(args[i + 1]).toBe('you are mandarax')
    })
...
- [ ] **Run** (expect FAIL):
  ...
  ```
  pnpm --filter @mandarax/harness test codex-args
  ```
...
- [ ] **Minimal impl** — `packages/harness/src/codex/args.ts` (replace `// VERIFY` literals with the Task-5 confirmed names). Note: `systemPrompt:'flag'` means core passes the prompt **text** (not a file path) as `turn.systemPrompt`:
  ...
  // risky ops via its own sandbox, so mandarax registers no permission gate for it). System
...
- [ ] Replace the placeholder `packages/harness/src/codex/codex.ts` with the verified capabilities:
  ...
  // ops via its own sandbox, so mandarax wires no /permission route for it. systemPrompt:'flag'.
...
- [ ] **Run PASS** + typecheck:
  ...
  ```
  pnpm --filter @mandarax/harness test codex && pnpm --filter @mandarax/harness typecheck
  ```
...
- [ ] **Run** (expect FAIL — placeholders return `[]` from `buildArgs`, don't throw):
  ...
  ```
  pnpm --filter @mandarax/harness test stubs
  ```
...
- [ ] Write the **failing IT** `packages/harness/test/harness.it.test.ts`. It drives **any** adapter end-to-end through a real spawned `fake-harness` child whose output stream is named by an env var, and proves: (a) a generic adapter's `decode` produces RUN_STARTED…RUN_FINISHED from real child stdout; (b) a `permissionGate:'none'` adapter carries no permission wiring (no `--settings`, no `permissionUrl` consumption) — the gate is skipped cleanly:
  ...
  function spawnFake(format: 'claude' | 'codex'): Readable {
    const child = spawn(process.execPath, [fakeHarness], {
      ...
      env: {...process.env, MANDARAX_FAKE_FORMAT: format},
    })
    return child.stdout!
  }
...
- [ ] **Run** (expect FAIL — fixture missing):
  ...
  ```
  pnpm --filter @mandarax/harness test harness.it
  ```

  Expected: FAIL — `fixtures/fake-harness.ts` not found / spawn error.

- [ ] **Minimal impl** — `packages/harness/test/fixtures/fake-harness.ts`. A real executable (NOT a JS mock — spawned as a child to exercise the true stdout-pipe path), generalizing `fake-claude.ts`: it picks the output transcript by `MANDARAX_FAKE_FORMAT` (`claude` | `codex`), echoes argv to `MANDARAX_TEST_ARGV_FILE` when set, and hangs until SIGTERM under `MANDARAX_FAKE_HANG`. Use **named functions**, no IIFE. The `codex` lines must match the verified Task-5 event shape:
  ...
  // selected by MANDARAX_FAKE_FORMAT. Echoes argv (MANDARAX_TEST_ARGV_FILE) so a test can assert
  // --resume; hangs until SIGTERM under MANDARAX_FAKE_HANG to exercise Stop.
...
- [ ] **Run** (expect FAIL only if an adapter violates the invariant; otherwise it passes immediately — in which case temporarily break claude's `claude.ts` by removing `parseHistory` to SEE it go RED, then restore). Standard cmd:
  ...
  ```
  pnpm --filter @mandarax/harness test capability-matrix
  ```
...
- [ ] **Run PASS** (after restore) the whole suite + typecheck + lint:
  ...
  ```
  pnpm --filter @mandarax/harness test && pnpm --filter @mandarax/harness typecheck && pnpm --filter @mandarax/harness lint
  ```
...
## Task 10 — Extract claude OUT of `@mandarax/core`; depend on `@mandarax/harness`

**Files:**

- `packages/core/package.json` (add `@mandarax/harness` dependency)
...
- [ ] **Run** the core suite to capture the GREEN baseline before the move:
  ...
  ```
  pnpm --filter @mandarax/core test
  ```

  Expected: PASS (Plan 1 left it green). This is the regression guard for the extraction.

- [ ] Repoint core's imports from its inline claude modules to `@mandarax/harness`:
  - `import {getHarness, listHarnesses} from '@mandarax/harness'` (the registry — package entry `.`)
  - The default-resolution helper now calls `getHarness(config.harness ?? 'claude')`.
  - The claude default system prompt: `import {CHAT_SYSTEM_PROMPT} from '@mandarax/harness/claude'`? — NO: keep core's `systemPrompt` resolution generic. If core needs a fallback prompt only when none is configured, read it from the resolved adapter's package via `@mandarax/harness/claude` **only inside the claude-specific default path**, or (preferred) keep core's existing generic default prompt and let the adapter's `systemPrompt` capability decide delivery. Choose the generic path to avoid coupling core to claude.
...
- [ ] **Run PASS** (no behaviour change — core now resolves claude from `@mandarax/harness`):

  ```
  pnpm --filter @mandarax/core test && pnpm --filter @mandarax/core typecheck
  ```
...
- [ ] **Commit**:
  ...
  git commit -m "refactor(core): resolve claude from @mandarax/harness; drop inline copy
...
- [ ] **Run** (expect FAIL — the route currently wires `/permission` + `/history` unconditionally and always sets `permissionUrl`):
  ...
  ```
  pnpm --filter @mandarax/core test chat-route
  ```
...
- [ ] **Minimal impl** — make `makeChatRoute` capability-aware. Capture the resolved `adapter` in `ChatRouteOpts` and branch on `adapter.capabilities`. Concrete edits to `packages/core/src/chat-route.ts`:
  ...
  4. In the `POST /api/chat` turn assembly, build the `HarnessTurn` with capability-driven fields:
     ...
     ```
     where `writeSystemPromptFile(lockDir, text)` writes the prompt to `<lockDir>/.mandarax/system-prompt.txt` and returns the path (move the existing claude temp-file write here if Plan 1 had it inline; it is now claude-capability-gated, not claude-specific). `opts.spawn` is the generalized `SpawnHarness` seam (rename from `SpawnClaude`):
     ```ts
...
- [ ] In the engine boot file, resolve the adapter once and pass it + the resolved system-prompt text into `makeChatRoute`:
  ...
  `defaultSystemPromptFor(adapter)` returns `CHAT_SYSTEM_PROMPT` for claude (import from `@mandarax/harness/claude`) and `''` otherwise — keep this the ONLY claude-name reference in core, isolated in one helper.
...
- [ ] **Run** the whole monorepo to prove nothing downstream broke:
  ...
  Expected: all packages green. Pay attention to `@mandarax/widget` browser ITs and `@mandarax/cli` — they consume core's routes and must be unaffected (claude path unchanged in behaviour).
...
- [ ] **Scope match** — `@mandarax/harness` exists with NO `index.ts`; subpath exports `.` (registry), `./claude`, `./codex`, `./gemini-cli`, `./opencode`, `./pi` all resolve to named files; registered in the pnpm workspace (`packages/*` glob) and installed.
- [ ] **Claude ported file-by-file** — `args.ts` (buildArgs), `decode.ts` (decode), `history.ts` (transcriptPath + parseHistory), `system-prompt.ts` (CHAT_SYSTEM_PROMPT) each have a test; the old inline copies in `@mandarax/core` are deleted (no duplication). Capabilities `{resume:true, permissionGate:'hook', transcriptHistory:true, systemPrompt:'file'}`.
...
- [ ] **Constraint A — every adapter via `defineHarness`** — `claude`, `codex`, and the `gemini-cli`/`opencode`/`pi` stubs are each `export const x = defineHarness({…})`, never a bare object literal; the in-test synthetic adapters (`stub()` in registry.test, `noGateAdapter()` in chat-route.it) also go through `defineHarness`. Every adapter file imports `defineHarness` from `@mandarax/protocol/harness-types`. claude's `transcriptHistory` stays `false` in the intermediate Task 2/3 forms and flips to `true` only in Task 4 alongside `transcriptPath`+`parseHistory`, so `defineHarness`'s module-load invariant never throws mid-port.
