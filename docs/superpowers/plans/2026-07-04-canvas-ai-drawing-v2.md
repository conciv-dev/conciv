...
- **GOTCHA (found in Task 5, applies to any new Jazz table):** every new table must be added to `packages/extensions/whiteboard/src/shared/permissions.ts` (`scopedTables`) or writes fail at runtime with `WriteError("policy denied INSERT on table …")` — the schema alone is not enough. `canvasDraftElements` + `canvasReplies` were added there in the Task 5 commit. Task 3's schema change should have included this; it did not, and the failure only surfaces in a browser IT (the server insert on `canvasPending` was already permitted, so Task 4 stayed green). If you re-run from scratch, fold the permissions edit into Task 3.
