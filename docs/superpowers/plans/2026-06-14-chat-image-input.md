...
**Goal:** Finish capability-gated image input for the conciv chat composer — drag-and-drop, paste, an upload button, and multiple images per message.
...
- [x] Delivery — `turn.ts` runs `chat({adapter: harnessText(harness, …)})`; claude `buildArgs` writes each image to `.conciv-img-<uuid>.<ext>` under `cwd` and appends `@<path>` to the prompt (`packages/harness/src/claude/args.ts`, c16d5dd).
...
- [ ] **Step 3: Edit `config-types.ts`** — extend `ConcivConfig`, export defaults:
...
```ts
import {IMAGE_DEFAULTS, type ImageLimits} from '@conciv/protocol/config-types'
...
```ts
import type {ImageLimits} from '@conciv/protocol/config-types'
...
import {IMAGE_DEFAULTS} from '@conciv/protocol/config-types'
...
<form class="pw-chat-composer" onSubmit={submit}>
  ...
  <div class="pw-chat-composer-row">
    ...
    <textarea
      ...
      aria-label="Message the conciv agent"
      value={input()}
...
Run: `pnpm turbo build typecheck --filter=@conciv/widget`
Expected: PASS.
...
it('shows the upload button and a thumbnail after selecting an image', async () => {
  ...
  await page.click('[aria-label="Open conciv chat"]')
...
Expected: PASS (rebuild required — the IT loads `dist/conciv-widget.global.js`).
...
- [ ] **Step 5: Manual smoke (real claude harness, end-to-end)** — open the widget, paste a screenshot, send "what's in this image?", confirm the agent describes it (claude reads the `@<path>` written under cwd). Confirm a `.conciv-img-*.png` is written and cleaned up as expected.
