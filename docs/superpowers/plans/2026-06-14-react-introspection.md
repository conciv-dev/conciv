...
**Dev-only safety:** The conciv widget is injected only by the dev plugin and only activates when its probe to the dev engine succeeds, so `bippy` (private React internals) never runs in production. No extra gating task; noted here intentionally.
...
```bash
pnpm --filter @conciv/widget add bippy@0.5.41
```
...
```bash
pnpm --filter @conciv/core add @jridgewell/trace-mapping@0.3.31
pnpm --filter @conciv/core add -D @jridgewell/gen-mapping
```
...
Run: `pnpm --filter @conciv/protocol exec vitest run test/react-verbs.test.ts`
...
Run: `pnpm --filter @conciv/protocol run typecheck`
...
async function chunkWithInlineMap(source: string, line: number, column: number): Promise<string> {
  ...
  const path = join(tmpdir(), `conciv-chunk-${Math.random().toString(36).slice(2)}.js`)
...
Run: `pnpm --filter @conciv/core exec vitest run test/page/symbolicate.test.ts`
...
Run: `pnpm --filter @conciv/widget run typecheck`
...
Expected: PASS — produces `packages/widget/dist/conciv-widget.global.js` containing bippy.
...
Run: `pnpm --filter @conciv/core exec vitest run test/api/page/page.it.test.ts -t "enriches a locate"`
...
Run: `pnpm --filter @conciv/core exec vitest run`
...
Run: `pnpm --filter @conciv/cli exec vitest run test/cli.it.test.ts -t "react verbs"`
...
Run: `pnpm --filter @conciv/widget exec vitest run test/widget.it.test.ts -t "locate returns"`
...
// Driving `conciv tools page locate` through the engine HTTP API proves the full chain:
...
(Confirm the TanStack example's engine port. If it differs from `41700`, read it from the example's `instrumentation`/config and substitute. If no engine is wired into the TanStack example yet, that wiring is a prerequisite — verify `@conciv/plugin` is registered in its `vite.config.ts` the same way the Next example wires it; if absent, add a Task 10a to wire it before this test.)
...
- **Open prerequisite flagged:** T10 assumes the TanStack example has the conciv engine wired (port + plugin). Verify before T10; add wiring as T10a if missing.
