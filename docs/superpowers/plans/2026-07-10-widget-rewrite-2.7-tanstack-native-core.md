...
## Amendment v3 (2026-07-11, user-locked: "no stores; expose the ORM for data, seal the run lifecycle")

Reshapes Task 1 and the db surface. Binding over everything below:

1. **`SessionStore`/`UiState` facades DIE** (files + types + tests). Core receives the typed drizzle instance (`db`) and writes raw queries for the plain-row tables — sessions/drafts/markers have no invariants; zod parses at the wire boundary only. `RpcDeps`/`ChatDeps` carry `db` instead of `store`/`uiState`.
2. **Run state moves OFF the sessions table** into PRIVATE tables `runs` (sessionId pk, status, runEpoch, lastError), `run_messages`, `replies` — defined in `packages/db/src/run-schema.ts`, which drizzle-kit sees but the package does NOT export. `SessionRecordSchema`/protocol are UNTOUCHED (no status/lastError/runEpoch fields anywhere public); the wire `running` flag derives from `statusOf`.
3. **`packages/db/src/run-queries.ts`** is the only door to run state — free functions taking `db`: `claimRun(db, id, kind)` (INSERT-or-ignore runs row, then atomic `UPDATE … WHERE status='idle'` + epoch bump + lastError clear + DELETE the session's run_messages/replies), `releaseRun(db, id, error?)`, `requestStop(db, id)`, `statusOf(db, id)`, `modelOf(db, id)`, `runEpochOf(db, id)` (sync reads), `setRunMessages`/`runMessagesFor` (JSON invariant), `writeReply`/`replyFor`, `clearRunState(db, id)` (session-delete hygiene). Bypass is structurally impossible: the run tables aren't importable outside `@conciv/db`.
4. **`watch()`/emit machinery dies entirely** — no store wrappers means no write hooks. The chat-plane doorbell (`makeChanges`, Task 2) is rung by EXPLICIT `notify()` calls at the ~5 writes that matter: run-pump row writes, claim/release/stop transitions, reply rpcs. CRUD writes notify nobody (nothing listens — Amendment v2 removed the live lists).
5. db package end state: `db.ts` (openDb + boot sweep: runs→idle, truncate run_messages+replies), `schema.ts` (three public tables, unchanged shape), `run-schema.ts` (private), `run-queries.ts`, `index.ts` (exports db/schema/run-queries ONLY). Tests: `run-queries.test.ts` + `db.test.ts` replace `session-store.test.ts`/`ui-state.test.ts`. Red window: core is red from Task 1 (facade deletion) until Task 2 rewires — Task 1's gate is db+protocol green only.
