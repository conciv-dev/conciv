...
## Amendment 2026-08-04 — delta pass against the post-rewrite tree (verified, file:line evidence in session)

Executed before Task 1 starts. Findings that BIND implementers:

1. **bin.ts assumption holds**: `packages/cli/src/bin.ts` has exactly `subCommands: {tools: toolsCommand}`; add `init` beside it. `open`/`page`/`react`/`server` are nested under `tools` (`packages/cli/src/tools.ts`), NOT root siblings — any plan prose implying root-level `open`/`page` is wrong; do not restructure them.
2. **Task 13 reshaped**: the claude connect artifacts exist but not as the plan assumed. Static skills plugin: `packages/harness/plugins/claude/.claude-plugin/` (via `findPluginDir()` in `packages/harness/src/claude/plugin-dir.ts`). Generated session plugin: `claudeConnectPluginFiles()` inline in `packages/harness/src/claude/attach.ts:195-212` (emits marketplace.json, plugin.json, bin bridge, .mcp.json — NO hooks.json; drop the plan's hooks claim). Install flow already shells `claude plugin marketplace add` + `claude plugin install conciv-connect@conciv --scope local` (`attach.ts:334-352`). Task 13's job is the EXTRACTION of that inline logic into a reusable module the init step calls — the flow itself is proven.
3. **`conciv/no-timers-in-tests` is NOT live**: the rule module exists (`packages/oxlint-plugin/src/no-timers-in-tests.js`, registered in the plugin) but is absent from the active `.oxlintrc.json`. Global Constraints line claiming it is live is corrected to: the BEHAVIOR is still banned by convention (no predicate-polling in tests); enabling the rule for `packages/cli/test/**` is added to Task 2's scope (one `.oxlintrc.json` override entry).
4. **`connect.plan()` does not exist**: the real terminal-initiated connect runtime is `runConnect()` in `packages/try/src/connect.ts:48-79` + `packages/protocol/src/connect-ports.ts`. Any task text referencing `connect.plan()` reads as `runConnect()`.
5. **Fixture corpus source correction**: e2e has 12 consumer apps (all vite-based except `nextjs`/`nextjs-component` on Turbopack) — see `e2e/README.md`. There are NO webpack or rspack e2e apps: `webpack-family.ts` fixtures (Task 10) must be hand-made, they cannot be seeded from e2e. Seed vite/nextjs fixtures from e2e as planned.
6. **Rename sweep (Task 14) is small**: the only real workspace dep on the bare `conciv` name is `packages/embed/package.json` (`"conciv": "workspace:*"`); plus `apps/conciv/.cta.json` projectName and doc mentions. `conciv-storybook` / `conciv-e2e-*` filter strings are DIFFERENT packages — do not touch them.
7. **Unchanged and verified**: guards.ts rejection of bare names (`assertValidPackageName`, `PUBLIC_PACKAGES`, the `startsWith('@conciv/')` scan filter), `@conciv/it` exports shape, `@clack/prompts ^1.7.0` precedent in packages/try, CLI `tools` self-description + dev-server resolution via `CONCIV_PORT`/`127.0.0.1:5173` (`packages/cli/src/request.ts`).
