...
- **Wrapper note (RESOLVED in CC0, no longer a constraint):** effect hosts are keyed dynamic attachments held outside the structural part set, so samePartsAs no longer compares them. Wrappers may bind an effect host in any order, before or after the four structural refs; a late binding re-homes the effect without re-registering the rig. Pinned by the `effects` check group.
