...
- **Head bob RESTORED:** CC0 gives activity a working-time head channel (donor values: yPercent −5, sine.inOut, yoyo) using the same handoff pattern as eyes/antenna; pose owns head when idle. Harness gains the bob check; the phase-1 changeset note about the dropped bob gets updated when this lands.
- **Harness in CI:** phase-1 branch wires `verify:behavior` into the package test script; phase-2 tasks extend the same CI-gated harness (per-task check groups already specified).
