...
- **Header id is canonical for all live state.** It is what the client sends in `aidx-session-id`: a
...
- **`aidx ui` (cross-process inject)** routes by the header id injected into the agent's spawn env
  (`AIDX_SESSION_ID`) and echoed back as the header.
- **`origin='aidx'`** iff a token appears in the map under a key **≠ itself** (new session: `uuid→token`;
  adopted external: `token→token`). This is true "started by aidx" and never poisons on resume.
...
1. **Foundation Tasks 1, 2, 3, 4, 5** — verbatim from the 2026-06-15 plan. They deliver: protocol header
   constants (`AIDX_SESSION_HEADER`, `DEFAULT_SESSION_ID`) + `harnessId`/`name` on `ChatSessionSchema` +
...
Run: `pnpm turbo typecheck --filter=@opendui/aidx-core && pnpm --filter @opendui/aidx-core exec vitest run`
...
- Modify: `packages/cli/src/cli-http.ts` (send `aidx-session-id` from `process.env.AIDX_SESSION_ID`)
...
Run: `pnpm --filter @opendui/aidx-core exec vitest run test/runtime/ui-bus.test.ts` → FAIL.
...
- In the spawn path (search for where `childEnv` / `AIDX_PORT` is set — `engine.ts`), make the child env per-turn and add `AIDX_SESSION_ID: <the turn's header id>`. This means `spawnHarness` must receive the header id (thread it from `turn.ts`'s `onSpawn`/spawn call).
- In `packages/cli/src/cli-http.ts`, add `aidx-session-id` from `process.env.AIDX_SESSION_ID` to the `/api/chat/ui` POST headers.
...
In `chat.it.test.ts` (or a new `ui-inject.it.test.ts`): start a turn for `h-a`, POST `/api/chat/ui` with the `aidx-session-id: h-a` header, assert the SSE for `h-a` contains the CUSTOM event and a concurrent `h-b` turn's SSE does not. (This is the cross-process path the unit test can't cover.)

- [ ] **Step 6: Run + typecheck**

Run: `pnpm --filter @opendui/aidx-core exec vitest run test/runtime/ui-bus.test.ts && pnpm turbo typecheck --filter=@opendui/aidx-core`
Expected: PASS.
...
- [ ] **Step 2:** `pnpm turbo typecheck --filter=@opendui/aidx-widget` → PASS.
- [ ] **Step 3: Commit** `git commit -m "fix(widget): compact turn carries the aidx-session-id header\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>"`
...
Expected: PASS (protocol has no in-package consumer of `claudeHistory`; `list` is optional). Note: `@opendui/aidx-harness` is unaffected because `list` is optional — B2 adds it.
...
Run: `pnpm --filter @opendui/aidx-harness exec vitest run test/claude-history.test.ts && pnpm turbo typecheck --filter=@opendui/aidx-protocol --filter=@opendui/aidx-harness`
Expected: PASS.
...
```ts
import {AIDX_SESSION_HEADER, DEFAULT_SESSION_ID, SessionId} from '@opendui/aidx-protocol/chat-types'
...
import {tmpdir} from 'node:os'
import {join} from 'node:path'
import {encodeProjectDir} from '@opendui/aidx-harness/claude/history' // or re-export
...
import type {ChatSessionMeta} from '@opendui/aidx-protocol/chat-types'
...
- [ ] **Step 4:** `pnpm turbo build --filter=@opendui/aidx-widget` → PASS. **Step 5: Commit**
...
- [ ] **Step 5:** `pnpm turbo typecheck build --filter=@opendui/aidx-widget` → PASS. **Step 6: Commit**
...
- [ ] **Step 4:** `pnpm turbo typecheck build --filter=@opendui/aidx-widget` → PASS. **Step 5: Commit**
...
- [ ] **Step 1: Extend the scripted server** with a `/api/chat/sessions` branch returning two canned rows (one `origin:'aidx'`, one `'external'`, distinct titles) and a `/api/chat/sessions/title` 200, and per-session `/api/chat/history` keyed by the `aidx-session-id` header.
- [ ] **Step 2: Write the IT (`browser.newPage()`):** open modal → selector shows both rows; the `origin:'aidx'` row shows the marker, the `external` one does not; click a row → a `/history` fetch fires with the new header and the thread swaps; "+ New session" → greeting + divider; rename → optimistic title shows; Tab from trigger→search→rename→rows never leaves the dialog; two mounted selectors don't share an `aria-controls` id. `page.close()`.
...
- [ ] **Step 2: Manual smoke** — on a repo with several Claude sessions: modal selector lists them with origin markers (aidx vs terminal), recency groups; switch loads history; rename persists across reload; split a qt pane onto a different session → both stream in parallel (no 409); generative-UI (`aidx ui`) from a qt pane renders in the right pane; reduced-motion honored.
...
- **Canonical-id consistency:** lock + uiBus + usage all keyed by header id (A6/A17/A18); list joins via the `previewId` map (B5); `aidx ui` routes by the env-injected header id (A17); `origin` = key≠value (B5). No uuid/token split remains.
...
- **Adaptation points (not placeholders):** B5/B12 test-server helpers (`getSessions`, `claudeHome` forwarding, header-aware `post`/`postChat`) extend the foundation's Task 7 helpers — read those first. B8's exhaustive body is specified by spec §5.2/§5.5/§6/§7. The A17 spawn-env change touches whatever module builds `childEnv` (search `AIDX_PORT`).
