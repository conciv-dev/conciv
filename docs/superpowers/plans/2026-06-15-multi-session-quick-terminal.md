...
- `chat-types.ts` (modify) — add `CONCIV_SESSION_HEADER` + `DEFAULT_SESSION_ID` constants; add `harnessId` + `name` to `ChatSessionSchema`; drop `sessionId` from `ChatRequestSchema`.
...
```ts
// POST /api/chat body — identity travels in the CONCIV_SESSION_HEADER, not the body.
...
Run: `pnpm --filter @conciv/core exec vitest run test/store/lock.test.ts`
...
const dirs: string[] = []
const tmp = () => {
  const d = mkdtempSync(join(tmpdir(), 'conciv-sess-'))
  dirs.push(d)
  return d
}
afterEach(() => {
...
Run: `pnpm --filter @conciv/core exec vitest run test/store/session-store.test.ts`
...
describe('sessionIdFromHeaders', () => {
  it('returns the header value when present', () => {
    expect(sessionIdFromHeaders(new Headers({'conciv-session-id': 'sess-a'}))).toBe('sess-a')
  })
  it('falls back to the default when absent or blank', () => {
    expect(sessionIdFromHeaders(new Headers())).toBe(DEFAULT_SESSION_ID)
    expect(sessionIdFromHeaders(new Headers({'conciv-session-id': '  '}))).toBe(DEFAULT_SESSION_ID)
  })
})
```
...
Run: `pnpm --filter @conciv/core exec vitest run test/api/chat/session-id.test.ts`
Expected: FAIL — module not found.

- [ ] **Step 3: Create `session-id.ts`**

```ts
import {CONCIV_SESSION_HEADER, DEFAULT_SESSION_ID} from '@conciv/protocol/chat-types'

// The session id a request targets: the CONCIV_SESSION_HEADER value, or the default session.
...
import {type H3} from 'h3'
import type {HarnessAdapter} from '@conciv/protocol/harness-types'
import type {ChatSession} from '@conciv/protocol/chat-types'
import {DEFAULT_SESSION_ID} from '@conciv/protocol/chat-types'
...
git commit -m "feat(core): per-session chat routes keyed by conciv-session-id header"
```
...
import {ChatSessionSchema, ChatHistorySchema, type ChatSession} from '@conciv/protocol/chat-types'
...
export function createChatApi(deps: {apiBase?: string; sessionId?: string} = {}): ChatApi {
  ...
  const sessionHeaders = (): Record<string, string> => (deps.sessionId ? {[CONCIV_SESSION_HEADER]: deps.sessionId} : {})
  return {
    base,
...
git commit -m "feat(widget): chat-api sends conciv-session-id header on all requests"
```
...
> This is what the live stream emits to the decoder. The decoder ignores `summary` (only `system`/`result` carry the session id), so existing decode tests are unaffected — verify by running `pnpm --filter @conciv/harness exec vitest run` after the edit.
