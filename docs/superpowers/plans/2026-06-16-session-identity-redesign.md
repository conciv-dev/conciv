...
describe('SessionId (branded, mandarax_ prefix)', () => {
  it('accepts an mandarax_ id', () => {
    expect(SessionId.safeParse('mandarax_018f...').success).toBe(true)
  })
...
Expected: FAIL — `SessionRecordSchema`/`ResolveRequestSchema` not exported; old `SessionId` accepts non-mandarax.
...
Delete `export const DEFAULT_SESSION_ID = 'default'`. Reshape `ChatSessionSchema`: rename `harnessId` → `harnessSessionId` (keep `z.string().nullable()`), and type `sessionId` as `SessionId`. Set `RenameSessionSchema.sessionId` to `SessionId`. Keep `MANDARAX_SESSION_HEADER`.
...
Run: `npx turbo run typecheck --filter=@mandarax/protocol`
...
Run: `pnpm --filter @mandarax/core add unstorage`
...
describe('SessionStore (memory driver)', () => {
  it('create → get round-trips', async () => {
    ...
    expect(await store.get('mandarax_a')).toEqual(rec)
  })
  it('update merges a patch and bumps updatedAt', async () => {
    ...
    const updated = await store.update('mandarax_a', {harnessSessionId: 'tok-1', title: 'Hi'})
...
import {SessionRecordSchema, type SessionRecord} from '@mandarax/protocol/chat-types'
...
describe('sessionIdFromHeaders', () => {
  ...
  it('rejects a malformed id → null', () => {
    expect(sessionIdFromHeaders(new Headers({'mandarax-session-id': 'no spaces!'}))).toBeNull()
  })
})
```
...
import {MANDARAX_SESSION_HEADER, SessionId} from '@mandarax/protocol/chat-types'

export function sessionIdFromHeaders(headers: Headers): string | null {
  const raw = headers.get(MANDARAX_SESSION_HEADER)
  if (!raw) return null
...
describe('resolveSession', () => {
  it('no id → mints a new record', async () => {
    ...
    expect((await d.store.get('mandarax_new'))?.origin).toBe('chat')
  })
...
import {ResolveRequestSchema, type SessionRecord} from '@mandarax/protocol/chat-types'
...
export async function resolveSession(deps: ResolveDeps, body: {id?: string}): Promise<{sessionId: string}> {
  ...
  // our id → return as-is
  if (body.id?.startsWith('mandarax_')) {
...
Run: `npx vitest run packages/core/test/api/chat/agent-handoff.test.ts`
Expected: PASS.
Run: `npx turbo run typecheck test --filter=@mandarax/core`
...
describe('defineClient (real server)', () => {
  it('resolve() returns the branded id; the header is attached only after setSessionId', async () => {
    ...
    client.setSessionId('mandarax_x' as never)
...
(Add `PermissionDecisionSchema = z.object({renderId: z.string(), approved: z.boolean()})` to `@mandarax/protocol/chat-types.ts` in this step — it backs the approval gate.)

- [ ] **Step 4: Run, verify it passes + typecheck**

Run: `npx vitest run packages/widget/test/session-client.test.ts`
Expected: PASS.
Run: `npx turbo run typecheck --filter=@mandarax/widget`
...
Remove the `activeToken` signal and the `onSessionLabel` token plumbing added during debugging. Create `const client = defineClient({apiBase})`; seed `client.setSessionId(readStorage('mandarax-active-session', parseMandaraxId, null))` on mount; persist on change. `onNew` → `await client.resolve()` then `client.setSessionId(id)` + `writeStorage`. Pass `activeId={() => client.sessionId()}` to the selector.
...
Each pane creates `defineClient({apiBase})`; pane "new" → `resolve()`; pane layout persistence (the `mandarax-qt-panes` list) stores our ids. Remove harness-id surface keying. Replace the direct `fetch(DELETE /api/chat/session)` in `forgetSession` (`quick-terminal.tsx:71`) with the closing pane's `client.remove()`.
...
Add `POST /api/chat/session/resolve` to the existing real test server: no id → return a fresh `mandarax_` id and remember it (stateful in the server closure); `mandarax_` id → echo; harness id → return a deterministic `mandarax_` wrapper. Keep `/sessions` returning the two rows. The reload test: open chat, send a message (new session, server mints `mandarax_` via resolve), reload, reopen — assert the thread + selector title persist. Add a switch+reload test and an adopt-external test.
