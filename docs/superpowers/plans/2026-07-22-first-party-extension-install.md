...
- **Packing local packages for fixtures uses `pnpm pack`, never `npm pack`** — `npm pack` does not rewrite `workspace:^` protocols and installs fail with `EUNSUPPORTEDPROTOCOL`. After packing, assert the tarball's `package/package.json` contains no `workspace:` string, and install tarballs into a fixture closed via overrides mapping every `@conciv/*` name to `file:<abs>.tgz` (a plain `file:` direct-dep install silently resolves transitive `@conciv/*` ranges from the registry — the 0.0.x line IS published). **pnpm ≥11.7 dropped the package.json `pnpm.overrides` field** (Task 4 finding): write the overrides in a FIXTURE-LOCAL `pnpm-workspace.yaml` instead, and do NOT pass `--ignore-workspace` (it would discard that file); the fixture's own workspace file also stops pnpm walking up to the repo workspace. Fixtures live under `os.tmpdir()`, never inside the repo tree. The fixture test/script must assert lockfile closure (every `@conciv/*` resolves `file:`).
