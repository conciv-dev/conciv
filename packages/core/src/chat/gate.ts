...
export function makeRunGate(deps: RunGateDeps): PermissionGate {
  const request = async (
    ...
    const toolCallId = detail.toolCallId ?? approvalId
    await ensureToolCallPart(deps, detail.toolName, toolCallId)
    deps.processor.processChunk(
      aguiApprovalRequestedFor({toolCallId, toolName: detail.toolName, input: detail.input, approvalId}),
    )
    return (
      (await awaitReply(
  ...
  return {
    decide: async (toolName, toolInput, _sessionId, toolUseId) => {
      ...
      return (await request(deps.sessionId, {toolName, input: toolInput, toolCallId: toolUseId})) ? 'allow' : 'deny'
    },
    request,
  }
}
