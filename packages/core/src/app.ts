...
import {askUi, makeConcivSandbox, type PermissionGate} from './chat/gate.js'
import {
  ensureChatRecord,
  makeCompactor,
  makeSend,
...
export async function makeApp(opts: MakeAppOpts): Promise<MadeApp> {
  ...
  const approvalGates = new Map<string, PermissionGate>()
      ...
      const result = await extension.__server?.({
        ...
        approvals: {
          request: (sessionId, detail) =>
            approvalGates.get(sessionId)?.request(sessionId, detail) ?? Promise.resolve(false),
        },
      })
