...
const MISSING_SESSION =
  "no session scope is established on this call path. Every effectful conciv runtime call runs inside runWithSession(scope, ...), established at a transport boundary (the rpc sessionOs middleware, the mcp request handler, a tool execution, or a code-mode run). A callback registered outside a request — a socket message handler, an emitter listener, a timer, a stream pump — does not inherit the caller's scope: capture the scope at registration time instead of calling session() inside the callback."
