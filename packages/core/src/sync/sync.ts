...
import {ORIGIN, type SnapshotStore, type SyncEngine} from '@mandarax/protocol/sync-types'
...
export type SyncUpgrade = (request: Request, room: string) => boolean
export type SyncOptions = {store: SnapshotStore; validateUpgrade?: SyncUpgrade}
export type Sync = {engine: SyncEngine; hooks: Partial<Hooks>}

type AwarenessChange = {added: number[]; updated: number[]; removed: number[]}
type RoomState = {doc: Y.Doc; awareness: Awareness; peers: Set<Peer>; controlled: Map<Peer, Set<number>>}

function roomOf(url: string): string | null {
  const {pathname} = new URL(url)
  if (!pathname.startsWith(SYNC_PREFIX)) return null
  return decodeURIComponent(pathname.slice(SYNC_PREFIX.length)) || null
}

function defaultUpgrade(request: Request): boolean {
  const host = (new URL(request.url).host || '').split(':')[0] ?? ''
  return originAllowed(request.headers.get('origin'), new Set()) && LOOPBACK.has(host)
}
...
export function createSync(opts: SyncOptions): Sync {
  const validate = opts.validateUpgrade ?? defaultUpgrade
  ...
  const hooks: Partial<Hooks> = {
    upgrade: (request) => {
      ...
      if (room === null || !validate(request, room)) return new Response('forbidden', {status: 403})
    },
    open: (peer) => {
      const room = roomOf(peer.request.url)
      if (room === null) return
    ...
    message: (peer, message) => {
      const room = roomOf(peer.request.url)
      const state = room === null ? null : rooms.get(room)
      if (!state) return
