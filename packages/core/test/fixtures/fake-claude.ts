...
// stream-json transcript. With MANDARAX_FAKE_HANG it sleeps until SIGTERM to exercise Stop.
...
if (process.env.MANDARAX_FAKE_HANG) {
...
} else if (process.env.MANDARAX_FAKE_PARTIAL) {
  ...
  process.exit(0)
} else if (process.env.MANDARAX_FAKE_RICH) {
    ...
    {type: 'summary', summary: 'Fake session title'},
    {
      ...
      message: {
        content: [
          ...
          {type: 'tool_use', id: 'tc1', name: 'mandarax_page', input: {verb: 'route'}},
        ],
      },
    },
  ...
  // input_tokens is configurable per spawn (MANDARAX_FAKE_INPUT_TOKENS) so interleaved-turn tests can
  // prove usage is keyed per session and not cross-written. Defaults to 100.
  const inputTokens = Number(process.env.MANDARAX_FAKE_INPUT_TOKENS ?? '100')
  const lines = [
    {type: 'system', subtype: 'init', session_id: 'sess-fake', model: 'claude-test'},
    {type: 'summary', summary: 'Fake session title'},
    {
