...
test('every page error code maps onto a declared rpc error name, and only the gate error stays unclaimed', () => {
  expectTypeOf<keyof typeof PAGE_ERROR_NAME>().toEqualTypeOf<PageErrorCode>()
  expectTypeOf<(typeof PAGE_ERROR_NAME)[PageErrorCode]>().toEqualTypeOf<
    Exclude<RegistryCallErrorName, 'APPROVAL_DENIED'>
  >()
})
