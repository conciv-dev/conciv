...
export function agentsSection(consented: HarnessId[]): string {
  ...
  return [
    ...
    '## conciv',
    '',
    'conciv connects coding agents to the app running in the browser through code mode: discover',
    'capabilities from `external_catalog`, call them from generated code. Skill: `conciv/skill.md`.',
    'Needs the dev server running.',
    '',
    'More conciv skills (setup, extension authoring, harness adapters, debugging): run',
    '`pnpm dlx @tanstack/intent@latest list` then `load @conciv/skills#<name>`; if intent is not',
    'available, read them directly under `node_modules/@conciv/skills/skills/`.',
    ...setUpFor,
    ...notes,
    '',
    endMarker,
