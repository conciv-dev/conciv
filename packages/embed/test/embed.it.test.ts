...
describe('embed boots the conciv app against a real core', () => {
  ...
  it('a reloaded page outranks the previous page in-flight write when both clocks read the same', async () => {
    ...
    try {
      ...
      expect(
        (await kit.rpc.navigation.set({entries: [{href: '/'}], index: 0, updatedAt: frozen + 5_000})).applied,
      ).toBe(true)
  ...
  it('renders catalog and code-mode parts without blanking the transcript', async () => {
    const page = await openPage()
    try {
      await openPanel(page)
  ...
  it('renders the new tool cards for results that do not match their payload schema', async () => {
    ...
    try {
      ...
      await expectLocator(page.getByText('Capability catalog').last()).toBeVisible({timeout: 30_000})
