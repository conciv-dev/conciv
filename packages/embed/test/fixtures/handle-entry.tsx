...
import {pageSpike} from './page-spike.js'

declare global {
  interface WindowEventMap {
    'spike:connect': CustomEvent<{base: string}>
  }
}
...
function SpikeConnectPane(): JSX.Element {
  const connect = getHostApi().useConnect()
  makeEventListener(window, 'spike:connect', (event) => connect.found(event.detail.base))
  return (
    <output
      aria-label="spike connect pane ready"
      style={{position: 'fixed', bottom: '0', left: '0', 'pointer-events': 'none', opacity: '0'}}
    >
      ready
    </output>
...
const spikeConnectGate = defineExtension({
  name: 'spike-connect-gate',
...
export function makeSpikeHandle(apiBase: string): ConcivHandle {
  return createConciv({extensions: [pageSpike], apiBase, settings: {defaultOpen: true}})
}

export function makeSpikeConnectHandle(): ConcivHandle {
  return createConciv({extensions: [pageSpike, spikeConnectGate], settings: {defaultOpen: true}})
}
