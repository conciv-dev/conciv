...
test.describe('holdFirstNavigationWrite holds one websocket frame without stalling the socket', () => {
  ...
  test('queues a second navigation write behind the retained one and flushes both in order', async ({page}) => {
    const server = await startProbeServer()
    servers.push(server)
    const held = await bootHeldNavigationPage(page, server)
  ...
  test('releases only after the retained write is answered, not when the gate opens', async ({page}) => {
    const server = await startProbeServer()
    servers.push(server)
    const held = await bootHeldNavigationPage(page, server)
  ...
  test('forwards a navigation write that arrives after release instead of stranding it', async ({page}) => {
