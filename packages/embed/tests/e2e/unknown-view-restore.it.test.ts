...
test.describe('restoring a persisted panel route whose extension view is no longer mounted', () => {
  test('lands on the chat view instead of an empty pane', async ({page}) => {
    ...
    await expect(page.getByRole('button', {name: 'Recorder'})).toHaveAttribute('aria-pressed', 'true')
    ...
    await expect(page.getByRole('button', {name: 'Recorder'})).toHaveCount(0)
    ...
    await expect(statusBar.getByRole('button', {name: 'Chat', exact: true})).toHaveAttribute('aria-pressed', 'true')
    await until(async () => !(await currentHref(suite.kit())).includes('/recorder'), {
      hangGuardMs: 30_000,
      intervalMs: 100,
    })
