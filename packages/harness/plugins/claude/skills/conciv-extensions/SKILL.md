...
1. `conciv_extensions` with `verb: "catalog"` — see the surface (theme tokens, the six slots, client/server surfaces). Read it before writing.
2. `conciv_extensions` with `verb: "scaffold", kind, name` — get a typed skeleton.
