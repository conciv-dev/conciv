...
description: Use when the user asks about a React component on the live page, or you need to map a rendered element to its source file, read its props/hooks, dump the component tree, or find a component by name. Covers the aidx_page tool's locate/inspect/tree/find verbs. Reach for this instead of poking __REACT_DEVTOOLS_GLOBAL_HOOK__ or fiber keys via the eval verb.
---

# React introspection

The live page is a React app. The `aidx_page` MCP tool has four React verbs that read the fiber
...
Call `aidx_page` with a `verb` plus an element target: either a positional CSS `selector` or a
`ref` (from the latest `aidx_page` `snapshot`). Prefer `ref`; refs go stale on re-render.

## Verbs (the `verb` argument to `aidx_page`)
...
1. Ground yourself: call `aidx_page` with `{verb: 'snapshot'}` to get refs, or
