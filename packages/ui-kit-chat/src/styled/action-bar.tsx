...
const TRIGGER =
  'inline-flex items-center justify-center size-7 rounded-[var(--chat-radius-sm)] [background:transparent] [color:var(--chat-text-3)] cursor-pointer [transition:background_140ms_var(--chat-ease),color_140ms_var(--chat-ease)] hover:[color:var(--chat-text-hi)] hover:[background:var(--chat-fill-strong)] data-[state=open]:[background:var(--chat-fill-strong)] focus-visible:[outline:0.125rem_solid_var(--chat-accent)]'
const MENU =
...
const ITEM =
  'flex items-center gap-2 px-2.5 py-1.5 text-[0.75rem] rounded-[var(--chat-radius-sm)] cursor-pointer select-none [color:var(--chat-text-2)] hover:[background:var(--chat-fill-strong)] hover:[color:var(--chat-text-hi)] [outline:none]'
...
export function AssistantActionBar(): JSX.Element {
  return (
    <ActionBar.Root hideWhenRunning autohide="not-last" class="flex gap-0.5 anim-presence-in">
      <ActionBar.Copy
        render={(props) => (
          <TooltipIconButton {...props} tooltip="Copy" class="size-7">
            <CopySwap />
          </TooltipIconButton>
        )}
      />
      <ActionBar.Reload
        render={(props) => (
          <TooltipIconButton {...props} tooltip="Refresh" class="size-7">
            <RefreshCw size={ICON} />
          </TooltipIconButton>
        )}
      />
...
export function UserActionBar(): JSX.Element {
  return (
    <ActionBar.Root hideWhenRunning autohide="not-last" class="flex flex-col items-end anim-presence-in">
      <ActionBar.Edit
        render={(props) => (
          <TooltipIconButton {...props} tooltip="Edit" class="size-7">
            <Pencil size={ICON} />
          </TooltipIconButton>
        )}
      />
