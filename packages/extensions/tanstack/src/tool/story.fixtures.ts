...
export const STORY_FRAME_CLASS = 'p-4 w-[28rem] [background:var(--chat-bg)] [font-family:var(--chat-font)]'
...
export function storyErrorResult(message: string): ToolResultPart {
  return {type: 'tool-result', toolCallId: 's1', content: JSON.stringify({message}), state: 'error', error: message}
}
