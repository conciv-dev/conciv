...
const OVERLAY_RULES = [
  ...
  'ios_screenshot for the actual pixels.',
  'There is no dev server and no hot reload here: a Swift edit reaches the screen only after ios_build',
  'and then ios_run, so end every change with that loop and confirm it with ios_screenshot.',
  'You can build and drive the app through the ios.* tools instead of raw bash.',
  'Tools: ios_build (compile the native project), ios_run (boot the simulator, install, and launch),',
  'ios_screenshot (capture the current simulator screen as an image), ios_logs (recent device logs).',
  'There is no ios.viewHierarchy tool. To understand what is on screen, call ios_screenshot for the',
