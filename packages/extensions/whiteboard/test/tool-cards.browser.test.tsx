...
describe('CanvasOpCard (real browser)', () => {
  it('canvas_read renders an element count chip', async () => {
    mountToolCard(CanvasOpCard, {
      name: 'canvas_read',
      args: {scope: 'live'},
      content: JSON.stringify({elements: [{}, {}, {}], scope: 'live'}),
    })
    await expect.element(page.getByText('canvas_read')).toBeVisible()
    await expect.element(page.getByText('3 elements').first()).toBeVisible()
  })

  it('canvas_preview renders the result thumbnail', async () => {
  ...
  it('canvas_delete renders a red op chip and no success-only body', async () => {
    mountToolCard(CanvasOpCard, {
      name: 'canvas_delete',
      args: {elementId: 'el_9'},
...
describe('CommentOpCard (real browser)', () => {
  it('comment_create shows the text preview and cid chip', async () => {
  ...
  it('comment_list shows the count', async () => {
    mountToolCard(CommentOpCard, {
      name: 'comment_list',
      args: {scope: 'session'},
  ...
  it('comment_delete renders a red op chip', async () => {
    mountToolCard(CommentOpCard, {
      name: 'comment_delete',
      args: {cid: 'c_42'},
  ...
  it('pin_set_state shows the pin state', async () => {
    mountToolCard(CommentOpCard, {
      name: 'pin_set_state',
