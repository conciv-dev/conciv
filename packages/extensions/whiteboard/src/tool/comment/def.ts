...
export const commentCreateDef = toolDefinition({
  ...
  inputSchema: z.object({
    ...
    x: z.number().describe('Pin position: scene X coordinate (canvas space, as returned by canvas_read).'),
    y: z.number().describe('Pin position: scene Y coordinate (canvas space, as returned by canvas_read).'),
  ...
  meta: {
    ...
    hint: 'x/y are scene coordinates from canvas_read',
  },
  streamTitle: 'Leaving a comment',
  promptSnippet:
    'Use comment_create to leave a pinned note on the canvas for the user to see. x/y are scene coordinates in canvas space (use the x/y of an element from canvas_read to pin near it).',
})
...
export const commentReplyDef = toolDefinition({
  ...
  promptSnippet: 'Use comment_reply to answer a comment; the reply joins the same thread.',
})
...
export const commentReadDef = toolDefinition({
  ...
  promptSnippet: 'Use comment_read to see a comment and everything in its thread.',
})
...
export const commentListDef = toolDefinition({
  ...
  promptSnippet: 'Use comment_list to find existing comments before adding more; scope "session" for the current one.',
})
...
export const commentResolveDef = toolDefinition({
  ...
  promptSnippet: 'Use comment_resolve once a comment has been addressed.',
})
...
export const commentDeleteDef = toolDefinition({
  ...
  promptSnippet:
    'Use comment_delete to remove a comment the user no longer wants; deleting the first comment removes the whole thread.',
})
...
export const commentMoveDef = toolDefinition({
  ...
  inputSchema: z.object({
    cid: z.string(),
    x: z.number().describe('New pin position: scene X coordinate (canvas space, as returned by canvas_read).'),
    y: z.number().describe('New pin position: scene Y coordinate (canvas space, as returned by canvas_read).'),
  }),
  ...
  promptSnippet: 'Use comment_move to reposition a comment pin on the canvas; x/y are scene coordinates.',
})
...
export const pinSetStateDef = toolDefinition({
  ...
  promptSnippet: 'Use pin_set_state to lock a pin to its element or let it float at an offset.',
})
