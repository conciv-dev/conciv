import {splitProps, onCleanup, onMount, type JSX} from 'solid-js'
...
export function ConcivLockup(props: ConcivLockupProps) {
  ...
  let rootElement: SVGSVGElement | undefined
  ...
  onMount(() => {
    if (!local.interactive || !rootElement) return
    const link = rootElement.closest('a')
    if (!link) return
    focusTarget = link
    makeEventListener(link, 'focus', handleFocus)
    makeEventListener(link, 'blur', stop)
  })
...
export type ConcivMarkProps = {
  class?: string
}

export function ConcivMark(props: ConcivMarkProps) {
  ...
  return (
    <svg
      ...
      class={local.class}
    >
      <path fill-rule="evenodd" d={`${MARK_HEAD}${MARK_CURSOR}`} />
      <path stroke={MARK_COLOR} stroke-width="0.5" d={MARK_CURSOR} />
      <path d={MARK_ANTENNA} />
    </svg>
  )
}
