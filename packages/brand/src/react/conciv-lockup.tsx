...
function LockupSvg({className, cursorRef, antennaRef, rootRef, onPointerEnter, onPointerLeave}: LockupSvgProps) {
  return (
    <svg
...
function InteractiveLockup({className}: {className?: string}) {
  const reduceMotion = useReducedMotion()
  const cursor = useRef<SVGPathElement>(null)
  const antenna = useRef<SVGGElement>(null)
  const blink = useRef<AnimationPlaybackControls | null>(null)
  const flick = useRef<AnimationPlaybackControls | null>(null)

  const start = useCallback(() => {
    if (reduceMotion || blink.current) return
    if (cursor.current) blink.current = startBlinking(cursor.current)
    if (antenna.current && !flick.current) {
      flick.current = flickAntenna(antenna.current)
      flick.current.finished.then(() => {
        flick.current = null
      })
    }
  }, [reduceMotion])
  ...
  const bindFocusTrigger = useCallback(
    (node: SVGSVGElement | null) => {
      const link = node?.closest('a')
      if (!link) return
      const onFocus = () => {
        if (link.matches(':focus-visible')) start()
      }
...
export function ConcivLockup({className, interactive = false}: {className?: string; interactive?: boolean}) {
  if (interactive) return <InteractiveLockup className={className} />
  return <LockupSvg className={className} />
}

const MARK_HEAD =
  'M15.1 14.63H48.9A6.32 6.32 0 0 1 55.22 20.95V43.65A6.32 6.32 0 0 1 48.9 49.97H27.39L18 58.17L18 49.97H15.1A6.32 6.32 0 0 1 8.78 43.65V20.95A6.32 6.32 0 0 1 15.1 14.63ZM5.065 26.292H5.065A2.09 2.09 0 0 1 7.155 28.382V36.218A2.09 2.09 0 0 1 5.065 38.308H5.065A2.09 2.09 0 0 1 2.975 36.218V28.382A2.09 2.09 0 0 1 5.065 26.292ZM58.935 26.292H58.935A2.09 2.09 0 0 1 61.025 28.382V36.218A2.09 2.09 0 0 1 58.935 38.308H58.935A2.09 2.09 0 0 1 56.845 36.218V28.382A2.09 2.09 0 0 1 58.935 26.292ZM18.06 25.34L24.921 32.31L18.06 39.28L20.96 42.18L30.719 32.31L20.96 22.44Z'

const MARK_ANTENNA =
  'M41.815 4.9L44.205 4.9L44.205 16.63L41.815 16.63ZM40.02 4.9A2.99 2.99 0 1 1 46 4.9A2.99 2.99 0 1 1 40.02 4.9Z'

const MARK_CURSOR = 'M32.51 38.6L32.51 42.18L46 42.18L46 38.6Z'

export function ConcivMark({className}: {className?: string}) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 64 64"
      ...
      className={className}
    >
      <path fillRule="evenodd" d={`${MARK_HEAD}${MARK_CURSOR}`} />
      <path stroke={MARK_COLOR} strokeWidth="0.5" d={MARK_CURSOR} />
      <path d={MARK_ANTENNA} />
    </svg>
  )
}
