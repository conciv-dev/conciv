...
export const Default: Story = {
  render: () => (
    <ScrollArea.Root class="border border-chat-line rounded-chat-surface-md bg-chat-sunken h-40 w-64">
      <ScrollArea.Viewport class="p-3 h-full w-full">
        <ScrollArea.Content>
          <For each={Array.from({length: 40}, (_, i) => i + 1)}>
            {(n) => <div class="text-[0.75rem] text-chat-text-2 font-chat-mono py-0.5">Line {n}</div>}
          </For>
        </ScrollArea.Content>
      </ScrollArea.Viewport>
