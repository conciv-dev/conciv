import {z} from 'zod'
import type {TestError, TestRow, TestState} from '@aidx/protocol/test-types'
...
const ResultSchema = z
  ...
  .loose()
const SpecSchema = z
  .object({title: z.string(), file: z.string().optional(), tests: z.array(z.object({results: z.array(ResultSchema).optional()}).loose()).optional()})
  .loose()
type Suite = {file?: string; specs?: z.infer<typeof SpecSchema>[]; suites?: Suite[]}
const SuiteSchema: z.ZodType<Suite> = z.lazy(() =>
  z.object({file: z.string().optional(), specs: z.array(SpecSchema).optional(), suites: z.array(SuiteSchema).optional()}).loose(),
)
const ReportSchema = z
  .object({suites: z.array(SuiteSchema).optional(), stats: z.object({duration: z.number().optional()}).loose().optional()})
  .loose()
