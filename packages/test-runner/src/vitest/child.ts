...
// │ That dual role already caused a fork-bomb: the manager imported a value from here, which   │
...
// │ band-aids: the schema lives in ../child-protocol.ts (so nobody imports this for a value)   │
...
// The out-of-process vitest runner: a clean child (spawned by the driver with NODE_OPTIONS
...
function isLaunchedDirectly(): boolean {
  try {
    return realpathSync(fileURLToPath(import.meta.url)) === realpathSync(processArgv[1] ?? '')
  } catch {
    return false
  }
}
