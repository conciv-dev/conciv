...
// page-bus when the conciv routes are live. Auto-mounts on load; also exports mountWidget.
...
// conciv/extensions/*). Their server halves were collapsed by the bundler; here their theme applies
...
export function mountWidget(extensions: AnyExtension[]): void {
  ...
  // built-in like highlight works even when the chat probe below fails (plain app, no conciv server).
  ...
  // Chat + page-bus only exist on the conciv dev server. Probe the non-session /models route: a 2xx
