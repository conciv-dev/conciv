...
describe('dehydrate', () => {
  ...
  it('collapses past max depth into a drillable sentinel with size', () => {
    ...
    expect(out.a.b).toEqual({__conciv: 'object', size: 1, preview: '{…}'})
  })
  ...
  it('collapses Map/Set and class instances to named previews', () => {
    ...
    expect(out.m).toMatchObject({__conciv: 'Map', size: 1})
    expect(out.s).toMatchObject({__conciv: 'Set', size: 2})
    expect(out.store).toMatchObject({__conciv: 'class', name: 'Store'})
  })
})
