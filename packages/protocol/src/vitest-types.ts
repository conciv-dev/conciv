// Wire types streamed over /__pw/vitest/stream and returned by the run/status/list
// routes. Mirrored verbatim by the widget's vitest card — never guessed there.
// `parseFailure` is the one pure bit worth unit-testing: it maps a Vitest v4
// TestCase (shape below) to our flat TestError.
...
// The result returned by `devgent tools vitest run` (printed as the tool-result the widget
// renders as a card) and by the run route. Carries the full tree so it's self-contained.
export type RunResult = {summary: Summary; failures: TestError[]; tests: TestRow[]}

export type VitestEvent =
