...
export function MascotAntenna({follow, ...rest}: MascotFollowPartProps): ReactElement {
  const {claimPart} = useMascotContext()
  const token = useRef<ClaimToken>({}).current
  useIsomorphicLayoutEffect(() => claimPart('antenna', token, follow), [claimPart, token, follow])
