...
import {thoughtCloudEffect} from './core/effects/thought-cloud.js'
...
function MascotStage(props: StageProps): JSX.Element {
  ...
  createEffect(() => service.mountEffect('thought-cloud', thoughtCloudEffect))
...
\`thoughtCloudEffect\` mounts a comic-style thought bubble above the antenna tip: two small trailing dots lead
into a rounded paper bubble holding three ink dots that pulse in a 0.2s stagger (0.34s each, power1.inOut,
yoyo) while the whole bubble bobs \`±2.5px\` on a slow 1.5s sine float. Toggle **working** on to see it
mount; it stays anchored to the antenna tip as the robot leans. Ported verbatim from the \`ThoughtCloud\` story
prototype: same geometry, colors (paper \`#f7f4ef\`, ink \`#2f3142\`), and timing, scaled by the antenna factor
so it renders identically at 44px.
`

const meta: Meta<StageProps> = {
  title: 'mascot/Effects/ThoughtCloud',
  tags: ['autodocs'],
  ...
  argTypes: {
    ...
    working: {
      control: 'boolean',
      description: 'Activity overlay; the thought cloud only mounts while working',
    },
    follow: {
