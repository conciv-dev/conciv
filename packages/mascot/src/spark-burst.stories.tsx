...
import {sparkBurstEffect} from './core/effects/spark-burst.js'
...
function MascotStage(props: StageProps): JSX.Element {
  ...
  onMount(() => {
    ...
    service.mountEffect('spark-burst', sparkBurstEffect)
  })
...
Mechanical port of the donor "Radial spark burst" bubble effect (\`RadialSparkBurst\` in the mascot-gaze
branch's story-bubble-effects) onto the current effect-mount contract. A canvas anchored to the antenna tip
draws ten radial strokes every 620ms that ease outward with \`easeOutCubic\` and fade over 520ms, alternating
the donor's accent and spark colors, scaled by the antenna factor.

Toggle **working** to watch the burst loop start and the canvas fade back into the tip when activity stops.
`

const meta: Meta<StageProps> = {
  title: 'mascot/Effects/SparkBurst',
  tags: ['autodocs'],
  ...
  argTypes: {
    ...
    working: {control: 'boolean', description: 'Activity overlay: mounts and drives the spark burst effect'},
  },
}
export default meta
