...
function HeartStage(props: StageProps): JSX.Element {
  ...
  onMount(() => {
    ...
    service.mountEffect('heart', heartEffect)
  })
  ...
  return (
    <div
      ...
      style={stageStyle}
      role="img"
      aria-label="conciv robot mascot with a pixel heart effect anchored to its antenna"
    >
...
Pixel heart: a 7x6 pixel heart anchored to the antenna tip pulses scale between 1 and 1.22 on a
\`sine.inOut\` yoyo loop while \`working\` is true, and drains back into the tip when work stops.
Toggle **working** to start and stop the emitter.
`

const meta: Meta<StageProps> = {
  title: 'mascot/Effects/Heart',
  tags: ['autodocs'],
...
export const Playground: Story = {
  render: (args) => (
    <div style={frameStyle}>
      <HeartStage state={args.state} working={args.working} />
    </div>
  ),
}
