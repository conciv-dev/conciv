...
for (const [name, mount] of Object.entries(EFFECT_MOUNTS)) {
  test(`the ${name} effect drains its nodes, tweens and frame loop on the falling edge`, async ({page}) => {
    ...
    expect(result.working.frames, `${name} runs the frame loops its rendering needs`).toBe(result.idle.frames + loops)
    expect(result.draining.nodes, `${name} still owns its emitter node mid-drain`).toBe(result.working.nodes)
    expect(result.draining.frames, `${name} keeps animating while the staged exit runs`).toBe(
      result.idle.frames + loops,
    )
    ...
    expect(result.drained.frames, `${name} cancels every frame loop it started`).toBe(result.idle.frames)
  })
}
